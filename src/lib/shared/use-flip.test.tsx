...
function stub(el: HTMLElement, boxes: () => Box) {
  el.getBoundingClientRect = () => {
    ...
    return { top, left, right: left, bottom: top, width: 0, height: 0, x: left, y: top, toJSON: () => ({}) } as DOMRect;
  };
}
...
describe("useFlip", () => {
  it("inverts on BOTH axes, so a multi-column grid reorganizes correctly", () => {
    ...
    const { rerender } = render(
      <Harness keys={["a"]} orderKey="1" layout={layout} onNode={(k, el) => nodes.set(k, el)} />,
    );

    // The card moves right AND down: cell (0,0) -> cell (1,1) of a grid.
    layout.a = { top: 40, left: 300 };
    rerender(
      <Harness keys={["a"]} orderKey="2" layout={layout} onNode={(k, el) => nodes.set(k, el)} />,
    );
  ...
  it("leaves a full-width stack on Y only, so the event feed is unchanged", () => {
    ...
    const { rerender } = render(
      <Harness keys={["review"]} orderKey="1" layout={layout} onNode={(k, el) => nodes.set(k, el)} />,
    );

    layout.review = { top: 120, left: 0 };
    rerender(
      <Harness keys={["review"]} orderKey="2" layout={layout} onNode={(k, el) => nodes.set(k, el)} />,
    );
  ...
  it("★ prunes the rect of an unmounted key, so a returning item does not fly in from nowhere", () => {
    ...
    const nodes = new Map<string, HTMLElement>();
    const layout: Record<string, Box> = { a: { top: 0, left: 0 }, b: { top: 900, left: 0 } };
    const onNode = (k: string, el: HTMLElement) => nodes.set(k, el);

    const { rerender } = render(
      <Harness keys={["a", "b"]} orderKey="1" layout={layout} onNode={onNode} />,
    );
    // b leaves the set (a tag filter excludes it) — its stale rect must be dropped.
    rerender(<Harness keys={["a"]} orderKey="2" layout={layout} onNode={onNode} />);
    // b returns, now sitting near the top rather than at its old y=900.
    layout.b = { top: 60, left: 0 };
    rerender(<Harness keys={["a", "b"]} orderKey="3" layout={layout} onNode={onNode} />);
