...
export function overCapReducedEmail(opts: {
  ...
  return {
    ...
    html: layout(
      ...
      `<p>Because your account stayed over its limit, we removed your largest files to bring it back under your plan. They stay in Deleted until <strong>${opts.recoverableUntil}</strong>. You're over your limit, so upgrade or free up space first, then restore them from each event's Deleted section.</p>`,
      { href: opts.dashboardUrl, label: "Manage storage" },
    ),
  };
}
...
export function inactivityRemovedEmail(opts: {
  ...
  return {
    ...
    html: layout(
      ...
      `<p>Your event <strong>${esc(opts.eventName)}</strong> was removed after 6 months of inactivity (a free-account policy). It's still recoverable until <strong>${opts.recoverableUntil}</strong>: restore it yourself from your dashboard's Deleted filter. After that it's permanently deleted.</p>`,
