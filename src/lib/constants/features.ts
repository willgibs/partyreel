...
// Single source for marketing feature copy — the home teaser (FeatureHighlights)
// and the /features page both read from here, so capability claims never drift.
// File-limit numbers are derived from the universal limits (lib/media/limits.ts)
// so they can't contradict what the uploader actually enforces.
...
export type Feature = {
  icon: LucideIcon;
  title: string;
  /** Short line — used on the home teaser grid. */
  body: string;
  /** Fuller paragraph — used on the /features page. */
  longBody: string;
...
// Home FeatureHighlights teaser = the curated `featured` subset (single source —
// no copy duplicated between the home grid and the /features page).
export const HOME_FEATURES: Feature[] = FEATURE_GROUPS.flatMap(
  (group) => group.features,
).filter((feature) => feature.featured);

// The highlight reel — the product's namesake payoff. Shared by the home
// ReelTeaser and the /features marquee so the story is told once. Written as a
// shipped capability (see ROADMAP for build status; copy finalizes pre-launch).
