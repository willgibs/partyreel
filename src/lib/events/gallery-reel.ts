 ...
 * (`events.reel_style_id`), the platform lever (`ops_flags.live_reel_enabled`), and, for the clip
 * creator, what the host's plan allows (video, the watermark, the clip's length cap). Everything
 ...
 * `media.reel_eligible` ("plays in the live reel", false only for a clip someone added to the
...
/** What the host's plan lets the on-device clip creator do. */
export type ClipFacts = {
  /** A video may be saved to the album (the clip is a video): paid plans only. */
  videoAllowed: boolean;
  /** The free plan's small mark on a clip. */
  watermark: boolean;
  /** The clip's length cap in seconds (tiers.ts's MAX_REEL_SECONDS, mirroring tier_limits()). */
  maxSeconds: number;
};
...
export function clipFactsForTier(tier: Tier): ClipFacts {
  return {
 ...
 * guess either stamps a paying host's clip with the free mark or lifts it off a free one.
 */
