...
// Newsletter opt-in capture, server-mediated. capture_guest_email is service-role-only, so this
...
//
// ★ THE CRACK THIS CLOSES. `capture_guest_email` writes into `guests.email`, the column whose single
// invariant is "CONFIRMED, the row's own account's address": the forensic row and the uploader
// resolver read it as proof, and the host's credit prints it beside a verified guest's name. A
// session with `user.email` set and `email_confirmed_at` NULL is an UNCONFIRMED sign-up: a perfectly
// real user id, an address nobody has proved, and without this gate it could walk an unproved
// address straight into the proved column through this route. `email_confirmed_at` is the ONLY thing
// that means verified anywhere, so it is the test here too. The function holds the same line on its
// own side (the guests grant tidy, 20260922213000): it writes only on a row whose own account is the
// confirmed owner of the address, so on a shared phone, where the token names whoever joined last,
// this session's address cannot land on that person's row. An unproved address has its own home
// (`guests.pending_email`, set through /api/guests/email) and reaches `guests.email` only by a claim
// that proves it.
