...
/**
 * "THE EARLIER RULING STANDS" RIDES AS AN ORDINARY CLAUSE (Will, 2026-09-19).
 *
 * ★ NOTHING HERE LEARNS A THIRD STATE, and that is the design. The reserved
 * word is the CHOICE, so every rule this module already enforces applies to it
 * unchanged: it rides once, it is dropped when the ledger holds it, and it
 * rides again the moment he changes his mind. A composer that special-cased it
 * would be a second place for the mechanism to drift from the ledger.
 *
 * ★ AND THE TRANSCRIBER LEARNS IT AT THE MERGE. Until the `stands` grammar
 * lands in `scripts/lab-review.mjs`, a paste carrying one is refused by name,
 * which is the honest failure: the desk composes what he answered and the tool
 * says it cannot record it yet. That is why no case here asserts against the
 * script (`lab-review.test.ts` runs the real parser and would fail on a word it
 * does not know yet).
 */
describe("an answer that stands by an earlier ruling", () => {
  ...
  it("composes as any other answer does, with its reason", () => {
    const out = composeSoFar(
      {
        answers: {
  ...
  it("is dropped once the ledger holds it, and rides again when overridden", () => {
    const key = holdId("light", 5, "depth");
    const transcribed: Transcribed = {
      answers: { [key]: { choice: "stands", note: "the earlier ruling stands" } },
      items: {},
      notes: {},
    };
    const held = { choice: "stands", note: "the earlier ruling stands" };
    expect(
      composeSoFar(
        { answers: { [key]: held }, items: {}, notes: {} },
        () => open(),
        transcribed,
      ).message,
    ).toBe("");
    // He changed his mind: the option he picked overrides the ruling, and the
    // clause rides as the replacement a later paste is for.
    expect(
      composeSoFar(
        {
          answers: { [key]: { choice: "lift", note: "the lift is better" } },
          items: {},
          notes: {},
        },
        () => open(),
    ...
    ).toBe('review light r5: depth=lift "the lift is better"');
  });
});
