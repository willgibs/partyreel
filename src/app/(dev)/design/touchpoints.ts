...
export const TOUCHPOINTS: Touchpoint[] = [
  ...
  },
  {
    ...
    decisionNote:
      "RULED (Will, 2026-08-28 + 08-31): law 3 (sampled where there is media, the ratified five where there is not), the engine cleared for promotion to globals.css, lights-on over lights-off, and the PALETTE: ours, globally, with border-beam's own colours reviewed side by side and not adopted. The doctrine grew a sibling for the vendored beam: SPILL is light from a lit thing falling on its surroundings, BEAM is an object lit at its own edge because it IS the live subject, a state that ends when the state does, with one named exception for a premium object at rest. Section 05 was corrected rather than kept: Will meant component design in general by 'the design underneath the beam', not the three cues, and the air blur is gone (hairline + lip, both flat). STILL OPEN: the corner. Section 04 now puts our sharp surface tokens against the library's 16px on the same card, because the beam is built for 16px and this system rounds surfaces to 2-4px, which makes an action the beam's natural layer. Also open: the frame cost of the three sweep drives, which needs a foreground window.",
  },
  {
    ...
    decisionNote:
      "REVIEWED in full (Will, 2026-08-31). Cleared: the hero underlight, the locked door, the doorbell arrival, awaiting-media, the album straddle, the QR plate switching on, the publish beat's sampled spill, the paper probe, the upload as light, and the whole-page scarcity test. KILLED: the pointer lamp (too loud on too common a surface), the CTA rim (border-beam does that job better on the premium buttons that wanted it), and the scan-through (its own ground-up visual-design round; the POUR is kept as a working technique with no placement rather than forced onto a surface). Moment 12 dropped upload-in-progress on review, since the staged opacity and progress bar already carry it. STILL OPEN: which of the QR plate's three readings to keep (beam, our own light, or both), and the corner question on the doctrine board's section 04.",
    variants: [
      "Hero underlight",
      "Locked door",
      "Doorbell arrival",
