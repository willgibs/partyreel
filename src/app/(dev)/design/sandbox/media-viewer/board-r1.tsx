...
/**
 * ROUND ONE'S EIGHT DECISIONS, KEPT AND UNIMPORTED (the desk re-cut,
 * 2026-09-24; `site-chrome`'s own precedent: `chrome.tsx`/`menu.tsx` after its
 * own round two). All eight are answered (`docs/reviews/media-viewer.json`)
 * and landing on the real component via `media-viewer-wiring` at the same
 * time as this round, so `board.tsx` no longer draws them: its `PREVIEWS` is
 * typed against round 2's own `asks`, which is `mine` alone, and a spec whose
 * asks shrank to one is what makes the old keys a type error there, exactly
 * as `SITE_CHROME`'s did at its own round two. Nothing here is dead in the
 * sense of wrong; it is dead in the sense of decided, kept for the record
 * rather than deleted the moment its own caller was (`export-flow`'s
 * `KeepsakeLead` names the same rule).
 *
 * The doc-comment on every function below is verbatim from round one's
 * `board.tsx`; only this paragraph and the previews map at its foot are new.
 */
...
export function openingScreen(shape: OpeningShape, s: BoardState) {
...
export function holdsScreen(shape: HoldsShape, s: BoardState) {
  const screen = screenFor(s);
...
export function whoScreen(shape: WhoShape, s: BoardState) {
  const screen = screenFor(s);
  return (
    <Screen
      id={`who-${shape}`}
...
export function nextScreen(shape: NextShape, s: BoardState) {
  const screen = screenFor(s);
  const { item, at } = shotFor(s);
  return (
    <Screen
...
export function closeupScreen(shape: ZoomShape, s: BoardState) {
  const screen = screenFor(s);
  const { item, at } = shotFor(s);
  return (
    <Screen
...
export function videoScreen(shape: VideoShape, s: BoardState) {
  const screen = screenFor(s);
  const origin = originOf(s.origin);
...
export function wayOutScreen(shape: WayOutShape, s: BoardState) {
  const screen = screenFor(s);
  const origin = originOf(s.origin);
  return (
    <Screen
...
export function linkScreen(shape: LinkShape, s: BoardState) {
  const screen = screenFor(s);
  return (
    <Screen
      id={`link-${shape}`}
