...
/* -- 4. THE REDUCED-MOTION GUARD -------------------------------------------
   Not a candidate: a hole, with its patch. tw-animate-css ships no
   reduced-motion guard, so every animate-in / animate-out utility on the site,
   the whole floating layer included, still runs for a reader who asked for less
   motion. Bible 14 is enforced by hand today and the floating layer is the one
   place it is not enforced at all.
