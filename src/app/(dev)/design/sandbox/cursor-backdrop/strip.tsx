 ...
 * own rendered height as a share of the home page's 11,092 px, read off the
 * page in Chrome at 1440x900 on 2026-09-18 (the method is in the track's
 * manifest). A band that is wrong here would argue for a rhythm the page does
 * not have. The hero is `min-h-screen`, so its share moves with the window; at
 * a reviewer's 900 px screen it is the 900 below.
 */

/** Section → the words on the band, and its share of the page at 1440. */
const BANDS: Record<HomeSectionId, { name: string; share: number }> = {
  "cinema-hero": { name: "The hero", share: 0.0811 },
  "trust-strip": { name: "Trust", share: 0.0078 },
  decomposition: { name: "Made from", share: 0.0742 },
  "film-strip": { name: "Film strip", share: 0.0673 },
  "no-app": { name: "No app, no account", share: 0.0718 },
  "full-quality": { name: "Full quality", share: 0.0482 },
  "live-demo": { name: "Live demo", share: 0.0798 },
  album: { name: "The album", share: 0.0824 },
  curation: { name: "Curation", share: 0.0692 },
  privacy: { name: "Privacy", share: 0.0768 },
  "reel-teaser": { name: "The reel", share: 0.098 },
  "events-teaser": { name: "Events", share: 0.0697 },
  "pricing-teaser": { name: "Pricing", share: 0.0626 },
  faq: { name: "Questions", share: 0.0683 },
  "cinema-close": { name: "Roll credits", share: 0.0428 },
};
    ...
    <div className="cb-strip dark" style={{ height }}>
      {rows.map((r) => (
        <div
          ...
          // The share as grow, from a zero basis: see `.cb-band` for why a
          // percentage height cannot work here.
          style={{ flexGrow: r.share / total }}
        >
          {r.kind === "photo" ? (
            <Image
              src={photo.src}
