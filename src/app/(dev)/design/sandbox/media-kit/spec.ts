 ...
 *
 * ★ EVERY COUNT AND EVERY PRICE IS INTERPOLATED, NEVER TYPED, and that is why a
 * pure-data file imports the board's data modules. It is this board's oldest
 * landmine (decision.ts): round three shipped a ruling surface whose first
 * sentence carried the one number the round existed to take back, because the
 * number was a word in a string. A number written into a sentence cannot be
 * wrong out loud; one interpolated from the batch can only be wrong if the batch
 * is. The cost is that `./sources` drags `./catalogue` into the registry's graph
 * on the lab's server routes, which are dev-only and already load this board.
 *
...
export const MEDIA_KIT = defineBoard({
  ...
  history: [
    {
      n: 4,
      date: "2026-09-15",
      changed: `Became a sourcing sheet on Will's note (places, not picks): ${SOURCES.length} real catalogues ranked by whether they hold a release, a plan that totals $${TOTAL}, and the finding that the refusal was of a TIER, not of a company.`,
    },
    {
      n: 3,
      date: "2026-09-15",
    ...
    },
    {
      n: 2,
      date: "2026-09-14",
      changed: `The exposure measured on the real site (${PRODUCTION_FILES} files, ${ROUTES.length} routes), all ${BRIDGE.length} posts mapped at the real geometry, the kit written as a call sheet, and four blocks the site can wear.`,
    },
    {
      n: 1,
      date: "2026-09-14",
  ...
  context: `All ${IDS_TOTAL} entries in MARKETING_IMAGES carry one line, "unsplash (per lab-pack comment; provenance unverified)", with no author, no source and no retrieval date, and the lab pack they were copied from is empty, so the trail is gone from the tree. Reading the licence settles it faster than a provenance hunt would: Unsplash's free terms exclude recognisable people, and all ${IDS_TOTAL} are full of them. Not a filing problem, a sourcing one. Three rounds then argued about ${IDS_TOTAL} positions on a page; Will's round-four note said the thing actually blocking him was not knowing where to go.`,

  verdict: {
    recommendation: `Say yes to the rule, buy a $${TOTAL} bridge that is genuinely released, rule on whether a crowd needs one, and shoot the kit at a real event we host. The route is still Mix.`,
    because: `The refusal at the heart of this track was of a TIER, not of a company. Unsplash's free licence excludes recognisable people; Unsplash+ is a different agreement on the same site, model and property released, warranted, perpetual for anything pulled inside a month, $${UNSPLASH_MONTH}. So the whole bridge is buyable for $${TOTAL} rather than unbuyable at any price.`,
    overrule: `Answer the crowds ask "all-faces" and the free half of the sheet is decoration, including the ${WEBSUMMIT_CC.toLocaleString("en-US")} conference photographs nothing paid can match.`,
  },
  ...
  asks: [
    {
      ...
      options: ["yes", "no"],
      recommended: "yes",
      because: `Author, source, the clause quoted, a retrieval date and a people field on every entry, and no recognisable face without a release. It runs on ${CANDIDATES.length} records already, with a suite that refuses one missing a field, and saying yes costs something: it bars four staged frames.`,
      evidence: "record",
    },
    {
      id: "spend",
      ...
      options: ["buy", "hold"],
      recommended: "buy",
      because: `One month of Unsplash+ plus ${HARD_FRAMES} iStock frames for the conference rooms, $${TOTAL} in total. Every visual in that month is model and property released, which is the exact clause the ${IDS_TOTAL} stand-ins never had, and a frame pulled inside the month stays licensed forever.`,
      overrule:
    ...
    },
    {
      ...
      because: `A risk call rather than a legal opinion, and worth ruling for what turns on it: Web Summit's ${WEBSUMMIT_CC.toLocaleString("en-US")} CC BY conference photographs and the whole Flickr corpus are crowds, and conferences are the one vertical no subscription on this sheet is deep in.`,
      evidence: "sheet",
    },
    {
      id: "kit",
      ...
      options: ["shoot", "park"],
      recommended: "shoot",
      because: `${MASTERS.length} masters, six per vertical, at a real event running Partyreel. One night closes nine of the twelve rows in the asset log, and the sheet sharpened the case: licensing the photographs is $${TOTAL} and licensing the films is $${CLIPS_IF_LICENSED} a year, so the money was never in the stills.`,
      evidence: "callsheet",
    },
  ],
  ...
  departures: [
    {
      id: "tier-not-company",
      from: "precedent",
      text: `Round one killed Unsplash on the sentence excluding recognisable people, and three rounds treated that as a refusal of the company. Unsplash+ is a different agreement on the same site: released, warranted, perpetual for anything pulled inside a month, $${UNSPLASH_MONTH}. Three rounds of "no stock we can name" had a $${TOTAL} answer the whole time.`,
      evidence: "plan",
    },
    {
      id: "bible-18",
      from: 18,
      text: `Bible 18 says no stock on a marketing surface, and it is already broken in production on a larger scale than round one reported: the ${IDS_TOTAL} are in ${PRODUCTION_FILES} production files and ${ROUTES.length} routes, and six of them sit in the chrome of all ${MARKETING_PAGES} marketing pages before a reader scrolls. That is the only reason a Licensed route exists at all.`,
      evidence: "exposure",
    },
    {
      id: "rank-by-release",
      from: "precedent",
      text: `Round three ranked sources by price. The free half is not cheap, it is unusable: every vertical this product sells into is a room full of recognisable faces, so a library with no release cannot supply the frames at any price. ${BARRED_SOURCES.length} of the ${SOURCES.length} places sit below a line for that one reason, including the two best free catalogues on it.`,
      evidence: "sheet",
    },
    {
    ...
    },
    {
      ...
      from: "precedent",
      text: `Licensed no longer survives its own column, and the board says so rather than leaving it an equal third. Under the rule it fills ${IDS_UNDER_RULE} of the ${IDS_TOTAL} ids and ${POSTS_UNDER_RULE} of the ${BRIDGE.length} posts, the two it cannot fill are the dance floor and the DJ, and it has nothing for the corporate half of the business. It stays to be walked, because walking it is what kills it.`,
      evidence: "bridge",
    },
    {
      id: "coverr-audited",
  ...
  assets: [
    {
      ...
      spec: `One month of Unsplash+ ($${UNSPLASH_MONTH}) and ${HARD_FRAMES} iStock Essentials frames for the conference rooms ($${HARD_FRAMES * ISTOCK_FRAME}), $${TOTAL} in total. A card, not a camera. It is what makes the bridge legal while the shoot is booked.`,
      replaces: "nothing; everything below is the shoot.",
    },
    {
      what: `${MASTERS.length} event photographs, six per vertical`,
      ...
      replaces: `all ${IDS_TOTAL} stand-ins by id.`,
      row: 7,
    },
    {
    ...
    },
    {
      ...
      spec: `3 to 5 s, 1080 x 1920, silent, each with its own poster, filmed at the same events, and the film cut from that footage (rows 4 and 1). The only clip licence on the sheet that survives cancellation is $${CLIPS_IF_LICENSED} a year, more than every photograph in the plan put together.`,
  ...
  sections: [
    {
      ...
      argument: [
        ...
        `The asymmetry is the argument. Licensing the photographs is $${TOTAL} and licensing the films is $${CLIPS_IF_LICENSED} a year, so the clips line costs more than every photograph here put together, and it is the one line a shoot deletes outright: a film of strangers cannot carry a product whose claim is that the frames came from the party you were at.`,
      ],
    },
    {
      id: "sheet",
      title: "The sourcing sheet",
      lede: `${SOURCES.length} real catalogues ranked by whether they hold a release, each with its price, the clause quoted word for word, and a contact sheet of the source's own thumbnails.`,
      argument: [
        ...
        `The contact sheets are hotlinked, never copied: ${SHEET_COUNT} pulls from the sources' own public search pages, ${FRAME_COUNT} frames in all, each recorded as the source's own thumbnail URL, so a watermarked comp stays a watermarked comp and nothing paid is ever in our tree. A source that draws no sheet prints its own measured reason rather than sharing an excuse.`,
      ],
    },
    {
      id: "surface",
    ...
    },
    {
      ...
      argument: [
        ...
        `The blog is swapped per POST as well as per id, because the sheet is per post: a file name can only carry an id, and fourteen of the ${POSTS_FILLED} filled posts take a different photograph from the one their cover's id is bridged with. Both namespaces ride every block, and apply.test.ts pins each post's rule to the route it belongs to.`,
      ],
      wiring: [
    ...
    },
    {
      ...
      lede: `Production files per id, recomputed from the tree by exposure.test.ts so a number here cannot go stale, plus the chrome that puts six of them on all ${MARKETING_PAGES} marketing pages before a reader scrolls.`,
      argument: [
        `Round one measured the blast radius as the blog. It is the site: ${PRODUCTION_FILES} production files across ${ROUTES.length} routes, with four ids in the footer strip and two in the nav panel, both of which live in the group layouts. Reach, rather than count, belongs to reception-table: it is the only id in both, so it is on every marketing page twice.`,
      ],
    },
    {
      id: "gap",
    ...
    },
    {
      ...
      lede: `All ${BRIDGE.length} posts, today beside what replaces it, at the real geometry of the surface it lands on, both rows cut at the same rung of the crop ladder.`,
      argument: [
        `Nobody hashed the covers. All ${BRIDGE.length} posts set cover: in frontmatter and 22 of them differ from what the fallback hash would give, so a person chose every miscast one out of eleven wedding and festival frames, which is why the conference post is a music festival. That changes what the fix is: ${BRIDGE.length} frontmatter lines, not ${IDS_TOTAL} files.`,
        `The second, harder search closed all four of round one's holes and staged ${CANDIDATES.length} frames in all, which moved the argument rather than winning it: 18 of the ${CANDIDATES.length} work only because nobody in them is recognisable, and the four with a face are the four that need a release nobody here holds. The frames worth anything to this product are the ones with faces in them.`,
        `The route is not two questions. It decides how many frames change and when, so the consequence is the table rather than a second word from Will: Mix swaps ${MIX_IDS} id and ${MIX_POSTS} covers now, Ours changes all ${BRIDGE.length} on one day, and Licensed fills ${POSTS_UNDER_RULE} of ${BRIDGE.length} under the rule rather than the ${POSTS_FILLED} it fills on paper.`,
      ],
    },
    {
      id: "callsheet",
      title: "The kit, as a call sheet",
      lede: `${MASTERS.length} masters, six per vertical, each naming what happens in the frame, where the camera is, what the light is doing and the crops it has to survive, so it can be shot from rather than argued with.`,
      argument: [
    ...
    },
    {
      ...
      lede: `The six fields the rule adds to a manifest entry, running on ${CANDIDATES.length} staged records with provenance.test.ts refusing one that is missing a field. The rule is not a proposal here; it is a suite you can watch pass.`,
      argument: [
  ...
  controls: [
    ...
    },
    {
      ...
      options: [
        { id: "all", label: "All" },
        { id: "weddings", label: VERTICAL_LABEL.weddings },
        { id: "birthdays", label: VERTICAL_LABEL.birthdays },
        { id: "corporate", label: VERTICAL_LABEL.corporate },
        { id: "festivals", label: VERTICAL_LABEL.festivals },
        { id: "trips", label: VERTICAL_LABEL.trips },
      ],
      default: "all",
    },
    {
  ...
  lookFirst: [
    ...
    },
    {
      ...
      note: `The staged batch on all ${BRIDGE.length} posts. Read the two red rows at the bottom: the conference post and the office party are the two the free corpus cannot fill at all.`,
    },
    {
      section: "bridge",
      state: { route: "mix" },
      note: `The recommendation, on the same posts. ${MIX_IDS} id and ${MIX_POSTS} covers change the week it is chosen; everything else is marked as owed rather than quietly left as it is.`,
    },
    {
      section: "callsheet",
  ...
  notes: [
    ...
    },
    {
      ...
      text: `Licensed fills ${POSTS_FILLED} of the ${BRIDGE.length} on the board and ${POSTS_UNDER_RULE} under ask 1, because ${BARRED_IDS.join(" and ")} carry a readable face and no release. The two numbers differ by exactly what the rule takes back.`,
    },
    {
      section: "surface",
