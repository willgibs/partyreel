/**
 * THE RATIFIED HOME ORDER (2026-08-25: the IA's album section split into
 * album + curation, pricing moved after the reel; 2026-08-26: privacy moves
 * up beside curation so the paper chapter is contiguous; 2026-09-01: two
 * quieter guest-side sections land above the live demo so chapter 1 winds
 * down before its anchor instead of escalating into the paper cut;
 * 2026-09-01 second pass, the adjacency rule: no two adjacent sections share
 * a layout, which changed section LAYOUTS, never this order or the chapter
 * map; 2026-09-18: the ORDER is untouched again and the CUT moved up one, so
 * full-quality wears a full-bleed photograph and ends chapter 1 while the
 * live demo opens the paper chapter it always led into). This
...
/**
 * THE CHAPTER MAP (the mixed-theme rule): each section's surface
 ...
 * pins it).
 *
 * ★ THE CUT MOVED UP ONE, and this line is the whole move. On the
 * switching photograph section: "we could use this to end the first
 ...
 * visual on the page after the hero, is the paper chapter's opener (a chapter
 * opens on a bespoke device, and a lit stage is one of them). The pair was already neighbours, so nothing came between them or was
