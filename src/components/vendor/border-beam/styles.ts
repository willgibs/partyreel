...
export const colorPalettes = {
  ...
  // PARTYREEL: a fifth palette, added so our hues can be A/B'd against theirs
  // from a single prop instead of a fork of this file. Every `pos` and `size`
  // below is copied verbatim from `colorful` so colour is the ONLY variable in
  // the comparison. Slots are assigned by nearest hue (their pink->our coral,
  // teal->green, magenta+purple->violet); our five are raised to the chroma
  // this effect actually uses (theirs sit near the sRGB gamut edge, ours ship
  // at oklch C 0.14-0.17), with hue held exactly. Values generated from
  // src/components/dev/glow-contrast.ts oklchToSrgb, not eyeballed.
  partyreel: {
    border: [
      { color: 'rgb(253, 0, 37)', pos: '33% -7.4%', size: '70px 40px' },
      { color: 'rgb(0, 120, 255)', pos: '12% -5%', size: '60px 35px' },
      { color: 'rgb(0, 189, 86)', pos: '2.1% 68.3%', size: '40px 70px' },
      { color: 'rgb(0, 189, 86)', pos: '2.1% 68.3%', size: '20px 35px' },
      { color: 'rgb(154, 14, 239)', pos: '74.4% 100%', size: '180px 32px' },
      { color: 'rgb(0, 120, 255)', pos: '55% 100%', size: '85px 26px' },
      { color: 'rgb(228, 156, 0)', pos: '93.9% 0%', size: '74px 32px' },
      { color: 'rgb(154, 14, 239)', pos: '100% 27.1%', size: '26px 42px' },
      { color: 'rgb(154, 14, 239)', pos: '100% 27.1%', size: '52px 48px' },
    ],
    spike: { primary: 'rgb(253, 0, 37)', secondary: 'rgba(0, 189, 86, 0.98)' },
    spikeLt: { primary: 'rgb(194, 0, 17)', secondary: 'rgb(0, 130, 60)' },
  },
};
...
// PARTYREEL: the pulse sizes we ship read only `colorPalettes`. This map serves
// the `sm`/`line` sizes we don't use, so `partyreel` aliases `colorful` here:
// it keeps the widened union type-checking without duplicating their data, and
// gives a sane fallback if anyone ever does reach for that combination.
export const smallColorPalettes = { ...smallColorPalettesBase, partyreel: smallColorPalettesBase.colorful };
...
// it keeps the widened union type-checking without duplicating their data, and
// gives a sane fallback if anyone ever does reach for that combination.
export const lineColorPalettes = { ...lineColorPalettesBase, partyreel: lineColorPalettesBase.colorful };
...
const lineInnerGradientDataBase = {
  colorful: [
...
// it keeps the widened union type-checking without duplicating their data, and
// gives a sane fallback if anyone ever does reach for that combination.
export const lineInnerGradientData = { ...lineInnerGradientDataBase, partyreel: lineInnerGradientDataBase.colorful };
...
// it keeps the widened union type-checking without duplicating their data, and
// gives a sane fallback if anyone ever does reach for that combination.
export const lineBloomColors = { ...lineBloomColorsBase, partyreel: lineBloomColorsBase.colorful };
