...
describe("the vendored border-beam package", () => {
  ...
  it("never hands the beam a hard-coded radius", () => {
    ...
    // handed its literal radius. The ring has to be whatever the object is:
    // a call site either omits `borderRadius` (the library reads the child's
    // computed corner) or passes one it MEASURED, as ProCardBeam does. The
    // glow boards that first pinned this retired on 2026-09-18, so the net is
    // every production call site now, found rather than listed.
    const sites = readdirSync(join(process.cwd(), "src/components"), {
      recursive: true,
    })
      .map(String)
      ...
      .filter((rel) =>
        readFileSync(join(process.cwd(), rel), "utf8").includes("<BorderBeam"),
      );
    expect(sites, "no BorderBeam call site found at all").not.toEqual([]);
    for (const rel of sites) {
      // Comments stripped first: a file that EXPLAINS the bug must not trip
      // the pin, or the next agent deletes the explanation to keep the fix.
      const src = readFileSync(join(process.cwd(), rel), "utf8")
        .replace(/\/\*[\s\S]*?\*\//g, "")
        .replace(/^\s*\/\/.*$/gm, "");
      const literals = [...src.matchAll(/borderRadius=\{\s*["'\d][^}]*\}/g)].map(
        (m) => m[0],
      );
      expect(literals, `${rel} hands BorderBeam a literal radius`).toEqual([]);
    }
  });
