...
/**
 * A QUESTION AN EARLIER RULING REACHED (Will, 2026-09-19).
 *
 * ★ WHAT IS PINNED IS THAT IT IS STILL A QUESTION. His whole ruling is that a
 * decision made later must not quietly delete an idea drawn earlier: "Don't
 * want an earlier selection to kill what could have been a better idea down the
 * road." So the badge is context, never a lock; the options are still on the
 * dock; and the one new thing is a THIRD answer beside "not clear to me", which
 * is the trash he asked for, recorded rather than skipped.
 */
const OVERTAKEN_ASK: AskStep = {
  kind: "ask",
  board: "light",
  boardTitle: "Light, shadow and lamp",
  round: 5,
  ...
  question: "In dark mode, how should two overlapping things be told apart?",
  options: [
    { id: "family", label: "The shadow family, as today" },
  ...
  overtaken: {
    by: "app-shape",
    badge: "Ruled since app-shape r1, 19 Sep: sharing is a sheet",
    line: "stands: the sheet rules where sharing is asked, never what it costs.",
    conceded: false,
    gloss: "'as today' here means before that ruling.",
  },
};

const CONCEDED: AskStep = {
  ...OVERTAKEN_ASK,
  askId: "register",
  question: "How strong should the aurora be?",
  options: [
  ...
  overtaken: {
    by: "glass",
    badge: "Ruled since glass r1, 19 Sep: every action lives in the lightbox",
    line: "concedes: his tiles rule puts every action in the one lightbox.",
    conceded: true,
    gloss: undefined,
  },
};

const stands = () => screen.getByRole("button", { name: /ruling stands/i });

describe("a step an earlier ruling reached", () => {
  it("badges it in place and still asks the question", () => {
    step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    expect(
      screen.getByText("Ruled since app-shape r1, 19 Sep: sharing is a sheet"),
    ).toBeInTheDocument();
    expect(screen.getByText(/^stands: /)).toBeInTheDocument();
    // The question and its options are exactly where they were.
    expect(
      screen.getByRole("heading", { name: /overlapping things/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /The shadow family/ }),
    ).toBeInTheDocument();
  ...
  it("glosses 'as today' only where an option still says it", () => {
    const one = step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    expect(screen.getByText(/'as today' here means/)).toBeInTheDocument();
    one.unmount();
    step("light.register", undefined, "light", [CONCEDED]);
    expect(screen.queryByText(/'as today' here means/)).not.toBeInTheDocument();
  });

  it("records 'the ruling stands' as an answer, with its reason", async () => {
    const user = userEvent.setup();
    step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    await user.click(stands());
    const held = getReviewStore().answers[holdId("light", 5, "depth")];
    expect(held).toEqual({
      choice: "stands",
      note: "the earlier ruling stands",
    });
    // And it composes as an ordinary clause, which is what the desk counts.
    expect(
      composeMessage([
        { board: "light", round: 5, ask: "depth", ...held, note: held.note },
      ]),
    ).toBe(
      'review light r5: depth=stands "the earlier ruling stands"',
    );
  });

  it("clears on a second press, like every other answer", async () => {
    const user = userEvent.setup();
    step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    await user.click(stands());
    expect(stands()).toHaveAttribute("aria-pressed", "true");
    await user.click(stands());
    expect(
      getReviewStore().answers[holdId("light", 5, "depth")]?.choice,
    ).toBeFalsy();
  });

  it("primes the button where the lane conceded", () => {
    const one = step("light.register", undefined, "light", [CONCEDED]);
    expect(stands()).toHaveTextContent("the lane concedes");
    one.unmount();
    step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    expect(stands()).not.toHaveTextContent("the lane concedes");
  });

  it("fills Next once the ruling is said to stand", async () => {
    const user = userEvent.setup();
    step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    expect(screen.getByRole("button", { name: /^Next$/ })).not.toHaveClass(
      "bg-foreground",
  ...
  it("offers nothing to stand by on an ordinary question", () => {
    step("light.depth");
    expect(
      screen.queryByRole("button", { name: /ruling stands/i }),
    ).not.toBeInTheDocument();
  });

  it("answers with s, and only where there is a ruling", async () => {
    const user = userEvent.setup();
    const one = step("light.depth", undefined, "light", [OVERTAKEN_ASK]);
    ...
    await user.keyboard("s");
    expect(
      getReviewStore().answers[holdId("light", 5, "depth")],
    ).toBeUndefined();
  });
});
