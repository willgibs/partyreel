...
4. **Build** — leave WHY-comments for the next agent; pull from the library (`/design`: the component index with each component's contract, the bible on `/design/rules`); reuse `src/components/ui` + `src/components/shared`; use the MCPs directly. **The design law is short on purpose: the bible (22 rules, Will's) and a component's contracts bind; everything else on the site is precedent you may break.** When a round asks for a page or a section, take the big swing: a different and better exploration beats a safe increment, prototyped in the lab first, then wired (Will, 2026-09-12). Design as if design resources are unlimited: Will makes any image, video, SVG, 3D or generative asset, so ask for exactly what the design needs in your manifest's Handoff and ship the manifest's stand-in meanwhile (Will, 2026-09-14). Go past "correct": **proactively propose a creative DELIGHT** (the "feels like magic" bar, our differentiator) — apply the `/emil-design-eng` skill + animate-by-frequency (high-freq instant, occasional standard, rare delightful). See [`docs/systems/design-system.md`](docs/systems/design-system.md) "craft guidance stack".
