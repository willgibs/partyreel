...
| `image-trail` | our own cursor-tracking image trail (the Codrops "Image Trail Effects" demo one as the reference, written from scratch: a photograph born on every stretch of cursor travel, sliding to the cursor and decaying behind it) with real homes on the marketing site, and `privacy-hero` round two on the same engine (tighter, faster, the trailing photographs decaying) | answered whole (`d140`, `long` at three seconds, `flick`, `s180`, `notfound`, `walks` on a new path); wiring in `trail-wiring` (the 404, banked in the Library); the board retires there |
...
| `album-hero` (round three) | answered in full (`none`, `w880`, `lg`, `settled`, `page`) | wiring in `album-wiring`; retires there |
| `river-visual` (round two) | answered (`card`, `in`, `ghost`; `proportion` withdrawn) | wiring in `river-wiring`; retires there |
