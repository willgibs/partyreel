...
| `admin-split` | the admin as its own Vercel project on this one repo, serving only the admin; the main project stops serving it; the cutover runbook for the Orchestrator | integrated at `7f3738ba`; the cutover's remaining steps are in `tracks/orchestrator.md`: the Vercel project is Will's dashboard step, then its env and the preview proof; the domain moves at the milestone |
