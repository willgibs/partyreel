...
| **The Vercel cost round** | **✅ 2026-09-11**: 381 retained deployments pruned to a few dozen and a tool that keeps them pruned; `launch-prep` builds only on `[preview]`; sharp out of the function trace (16.6 MB of 51.1 off every deployment, image optimization verified identical). The lab and admin subdomains are queued as architecture rounds, not savings. |
