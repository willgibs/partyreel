...
| **The home hero, round three (tracks `hero-scan`, `hero-burst`, `hero-river`)** | **✅ integrated 2026-09-14 (`66b4ebe`, `3aeef1a`, `ff0291a`)**: round two was ruled the same morning (the source: "if I scan this QR I get all of these images"; the reel and the gathering out); three variations off the source on `/design/c/home-hero?key=`, the source first as the reference: the scan (the cause in the frame), the burst (the origin in every direction), the river (the origin at the top). **Waits on your ruling**: which variation, its supporting elements and copy, the departures ruled in; then the wiring round. |
...
alias rebuilds (`/design/c?key=` is the desk; each board opens on its answer block and ends in asks worded so
