...
| **The Library x Lab round** | **🟡 Phases 0 to 2 landed 2026-09-15; your review by the desk is next**: Will stopped his round-four review before it began ("there's no cross-page navigation, each page's presentation can be super messy so it's hard for me to understand the work") and asked for a dedicated round that makes the lab an internal app: the **Library** at `/design/library` is the whole working rule set, levelled (law, contract, policy, program, guidance, precedent, proposal, ruling, landmine: [`design/README.md`](design/README.md)), global rules separated from component-exclusive contracts, Will's rulings in the repo ([`design/rulings.md`](design/rulings.md)); the **Lab** at `/design/lab` is the desk, the boards, proposals, tracks, the kit and the tools, with a fixed configurator, state in the URL and a copy-as-message review panel (the UI never writes; the Orchestrator transcribes his message into `docs/reviews/`). Phase 0 (the Orchestrator): the routes moved under one shell (top bar, sidebar, content, TOC; every old URL redirects with the key), the page templates, the link grammar, the markdown reader, the board spec type, the glossary, `pnpm lab:smoke`, `.mono` retired. Phase 1: `lab-shell`, `lab-rules`, `lab-kit`, `lab-library`, `lab-desk` in parallel on disjoint lanes (all five integrated the same day: `lab-library` `a76578de`, `lab-rules` `0fb9498e`, `lab-kit` `e00d3b00`, `lab-shell` `4ba890a5`, `lab-desk` `9fcea12d`; the weekly usage limit cut three off mid-build and they were resumed with their state spelled out); Phase 2, the migration wave, landed the same night (nine tracks: every standing board is two files on the kit, `1c0306dd` to `abe45329`; the shim and the legacy shared files deleted; the discipline test's exemption list empty). ★ The launch-prep alias still serves Phase 1 (`a0ef9867`: the shell, the rule layer, the desk, the light and rounding boards on the kit) because Vercel's cap of 100 API deployments a day was hit at the wave's close; the wave's build (`1b58863d`) is the next Orchestrator's first act after 2026-09-17 00:13 UTC, and until then the wave reviews on a local `pnpm dev` (`/design/lab`, no key needed locally). Phase 3 is your review of round four on the new surface: open `/design/lab?key=` on the launch-prep alias, walk the review session (every open ask of every board, `1`..`9` picks, one message at the end), paste the line in chat; the Orchestrator runs `pnpm lab:review` and the ledgers land under `docs/reviews/`. The first alias build showed the gate leaking (a keyless request answered 200 with the nav in the payload); the gate now runs in the proxy and the closed door is proven on a production build (`216830cc`). ★ Vercel's GitHub integration created no deployment for the two `launch-prep` pushes of the afternoon (agent pushes still register, as CANCELED); both alias builds were created through the Vercel API from the pushed SHA, so if a `[preview]` push ever fails to appear, that is the fallback. **Yours**: the first walk of the shell on the launch-prep alias. |
...
**Pending now:** your review of the boards after round four (the twelve tracks of 2026-09-15), on the local dev server until the
alias rebuilds (`/design/lab?key=` is the desk; each board opens on its answer block and ends in asks worded so
a ruling is a few words); the home hero's ruling (which variation off the source, its supporting elements and
copy, the departures ruled in). The asset asks are yours to produce whenever; stand-ins ship first. Everything
