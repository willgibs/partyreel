...
| **The review wave: the bible's second edition + twelve tracks, four rounds** | **🟡 round four integrated 2026-09-15; your review is next**: Will's rule-by-rule review (2026-09-14) rewrote nine rules and gave each a status on `/design/library/rules`; six lab boards (`palette`, `light`, `type-scale`, `floating-surfaces`, `brand-voice`, `media-kit`) and one production sweep (`kill-mono`, `69af90d`: no mono face in the product, bible 7 is the two-faces rule) ran in parallel with the rounding round on the Orchestrator's side, and every board came in the same day. Will's read at the close: one round of context was not enough for a real review, so **every board and hero variation ran a second and then a third round the same night, ten tracks at once** (the rounding board as an agent track), each round a workflow of build, a read-only review refuting the handoff, a fix and a second review; the shell gained the candidate block (`setCandidateCss`: a board hands the whole site the paste its ruling would land, rendered on the lab, the marketing pages, the host app, the guest surface and the admin portal), so every candidate is judged on the real pages. Round three merged: `palette` `b6da35d`, `light` `8d8d0af`, `type-scale` `5277925`, `floating-surfaces` `e7309d7`, `brand-voice` `778bdf1`, `media-kit` `db2af14`, `rounding` `93017e2`, `hero-scan` `1c2d0ea`, `hero-burst` `78e9538`, `hero-river` `39113bb`. ★ **Vercel's free-plan cap (100 deployments per trailing day) was hit at 23:31 on the 14th**, so the launch-prep alias still serves round one (`1668a55`) until the window frees (from 16:30 on the 15th); rounds two and three were verified on local production builds, and **the review surface until then is the local dev server** (`pnpm dev`, then `/design/lab?key=` for the desk). Will's overnight notes (2026-09-15, before sleep) opened **round four on twelve tracks at once**, and all twelve integrated by morning: the seven boards and the scan reopened on his notes (a fixed configurator on every board, previews at 1:1, more real UI, the app's UI open to any active lab track), the source released to `hero-source`, the new `hero-inflow` (the album streaming INTO the code), and the burst and the river killed as heroes and moved to their own boards (`album-hero`: the /features/album hero; `river-visual`: a feature visual in the bank). The shell gained the board dock and 1:1 stages for it. Round four merged: `palette` `c0d60b94`, `light` `bc2390f2`, `type-scale` `d762841f`, `floating-surfaces` `75be0b0d`, `brand-voice` `0414a120`, `media-kit` `1c2be959`, `rounding` `06ceeb41`, `hero-scan` `f4494d6e`, `hero-source` `797fa9ad`, `hero-inflow` `7294f1d5`, `album-hero` `f51fcdc5`, `river-visual` `3e81cc39`. The asks wait under Waiting on Will in [`tracks/orchestrator.md`](tracks/orchestrator.md); the asset log is [`ASSETS.md`](ASSETS.md). Then the composition pass (the ruled blocks stacked on the home arc and the dashboard), then the wiring rounds. |
...
| **The Library x Lab round** | **🟡 Phases 0 to 2 landed 2026-09-15; your review by the desk is next**: Will stopped his round-four review before it began ("there's no cross-page navigation, each page's presentation can be super messy so it's hard for me to understand the work") and asked for a dedicated round that makes the lab an internal app: the **Library** at `/design/library` is the whole working rule set, levelled (law, contract, policy, program, guidance, precedent, proposal, ruling, landmine: [`design/README.md`](design/README.md)), global rules separated from component-exclusive contracts, Will's rulings in the repo ([`design/rulings.md`](design/rulings.md)); the **Lab** at `/design/lab` is the desk, the boards, proposals, tracks, the kit and the tools, with a fixed configurator, state in the URL and a copy-as-message review panel (the UI never writes; the Orchestrator transcribes his message into `docs/reviews/`). Phase 0 (the Orchestrator): the routes moved under one shell (top bar, sidebar, content, TOC; every old URL redirects with the key), the page templates, the link grammar, the markdown reader, the board spec type, the glossary, `pnpm lab:smoke`, `.mono` retired. Phase 1: `lab-shell`, `lab-rules`, `lab-kit`, `lab-library`, `lab-desk` in parallel on disjoint lanes (all five integrated the same day: `lab-library` `a76578de`, `lab-rules` `0fb9498e`, `lab-kit` `e00d3b00`, `lab-shell` `4ba890a5`, `lab-desk` `9fcea12d`; the weekly usage limit cut three off mid-build and they were resumed with their state spelled out); Phase 2, the migration wave, landed the same night (nine tracks: every standing board is two files on the kit, `1c0306dd` to `abe45329`; the shim and the legacy shared files deleted; the discipline test's exemption list empty). ★ Vercel: the GitHub integration works (`a0ef9867` was its build); the wave's `[preview]` pushes were refused with "Deployment rate limited, retry in 24 hours" (the daily cap counts every path, the dashboard included), so the alias serves Phase 1 until 2026-09-17 00:13 UTC, when an empty `[preview]` commit rebuilds it (the API path stays the fallback). **Phase 3 opened 2026-09-15 as the clarity round and landed 2026-09-16** (`be1638f2` to the close): your first review through the desk answered three light asks (`kit=land`, `infusion=phase-1`, `register=accent`) and marked two not clear because the questions were labels with token options; so an ask now carries its context, where to look and options labelled in words wherever it is met (the board's answer block, the desk's session, the review card), `?` is an answer ("not clear to me", stored as a null choice and flagged on the desk), and every one of the 57 asks on the twelve boards is a question a stranger can answer, with the evidence labelled by the options' names (the light board by the Orchestrator, ten boards by one agent each; the ratchet `PLAIN` in `sandbox/registry.test.ts` is empty). The answering moved onto the board: `?session=<board>.<ask>` pins a review card under the dock that scrolls to the ask's evidence with its switches set, previews an option when picked, and hands the queue on board by board to the desk's one-message summary (`lab-review-card`, merged `0829c12d`). The palette board is the first catalog: twelve complete, named palettes as cards with swatches and the production Card, Button, Input, Badge and menu at true pixels in both modes, one dock control to pick, four asks instead of eight (`99544a2b`). Your directive is in [`design/rulings.md`](design/rulings.md) ("a question carries its context; an exploration is a catalog") and in the program, the guidance and the spawn prompt. The wave crashed the machine once (eleven agents' dev servers and builds on 36 GB of RAM; nothing was lost, and agents now run one process at a time, four at once). **Yours**: the review, on a local `pnpm dev` (`/design/lab`, no key needed): Start the review on the desk, answer each ask on its board, paste the one line the summary composes; the alias rebuilds after 2026-09-17 00:13 UTC (the close's `[preview]` push `c07eb8e3` was refused with the same rate limit, so the next Orchestrator pushes a fresh empty `[preview]` commit, or creates the deployment through the API, then smokes the alias with `pnpm lab:smoke --production --key` and prunes). |
| **The CI budget round** | **✅ 2026-09-15**: round four's twelve tracks ran the remote gate on every push (405 runs, about 1,500 GitHub Actions minutes on 09-14 and 09-15, 90 percent on `lp/*`) and spent the month's private-repo budget in two days; Will made the repo public for the interim. Policy now: CI is not the gate, the four local steps are; `ci.yml` runs on `main` and `launch-prep` pushes that touch code (docs-only pushes skipped, the Orchestrator's record commits say `[skip ci]`) and on PRs to `main`; an `lp/*` push runs it only on `[ci]` in the commit message. Vercel: an agent branch builds ONLY on `[preview]` since the storage round the same evening (fourteen handoffs in a day put deployment storage at 40 GB of the 10 GB month and function storage over; 103 deployments were deleted and the prune runs after every integration); the launch-prep alias is built once per round close. **Yours**: swap the repo back to private when the Actions budget clears. |
| **The home hero, round three (tracks `hero-scan`, `hero-burst`, `hero-river`)** | **✅ integrated 2026-09-14 (`66b4ebe`, `3aeef1a`, `ff0291a`)**: round two was ruled the same morning (the source: "if I scan this QR I get all of these images"; the reel and the gathering out); three variations off the source on `/design/lab/home-hero?key=`, the source first as the reference: the scan (the cause in the frame), the burst (the origin in every direction), the river (the origin at the top). **Waits on your ruling**: which variation, its supporting elements and copy, the departures ruled in; then the wiring round. |
...
| **MILESTONE-24: the reset on prod** | **✅ 2026-09-12 (`592da24`)**: the 22-rule bible and the component contracts on `/design/library/rules`, the look-pins and copy-pins gone, ★ landmines only, the big swing licensed. `design-gallery` and `home-hero` spawned the same day. |
| **The "less is more" reset** | **✅ 2026-09-12 (`f79a711`, `91606e9`, the docs pass)**: the design law is the 22-rule bible on `/design/library/rules` (Will's, ratified at the plan) plus each component's contract (a test tagged `@contract-for`, on the component's library row); 433 derived rules, the verdict island and the annotations layer gone; the look-pins and copy-pins deleted (no copy is pinned by a test); ★ means landmine only (92 stars to 33 across the two design docs); agents are told to take the big swing. Next: `design-gallery` and `home-hero` cut together; the rounding GUI round after the gallery. |
...
| **MILESTONE-22: rounds A and B on prod** | **✅ 2026-09-11 (`ffa12b6`)**: the rules bible, the library's specimens and index, the three hero registers, six single sources, the MonoCaption sweep. Next: round C, your three sittings (the rules pass on `/design/library/rules`, the rounding on the tuner, the light rulings on the boards), each landed by the Orchestrator; then round D closes the phase. |
| **The library phase, rounds A and B** | **✅ 2026-09-11 (`84cd975`)**: the rules bible on `/design/library/rules` (433 rules, verdicts pending), the library's missing specimens and the component index, the three hero registers, six single sources, the MonoCaption sweep. Milestone-22 next; then Will's three sittings (round C). |
...
`/design/lab?key=` on the launch-prep alias renders the open boards with their track previews.

**Pending now:** your review of the boards after round four (the twelve tracks of 2026-09-15) resumes on the
local dev server once the clarity round lands (`/design/lab` is the desk; every ask in plain words, answered
beside its evidence on the board, one line to paste at the end); the light board's six open asks are already
in plain words; the home hero's ruling (which variation off the source, its supporting elements and copy, the
departures ruled in) is four of those asks. The asset asks are yours to produce whenever; stand-ins ship first. Everything
else waits for the UI era to land on what it lands on (Will's ruling at milestone-23, 2026-09-12), unless
something blocks or is high-leverage; the parked list moved whole to Waiting on Will.
