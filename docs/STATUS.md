...
| `profile-page` | what a person is on Partyreel beyond one album: exists, the head, made of, identity, block, who is named, the claim, the list; phone first over one cast | answered whole (batch four); `profile-wiring` (Opus, cut `b30445d9`) lands the eight picks: the page on the album's header, one card group with a host or guest marker, a capped bio, block inside a Report menu, everyone named with the marketing sentence fixed, the claim right after an upload and free for everyone, a faces row with an interim View all; round two `profile-reach` (Sonnet) draws the three pieces he left open (how View all opens, the quick-look, the way back to the scanned event) |
...
| `site-chrome` | the marketing header, the mega panel, the phone's menu and the footer: the shape, what it holds, the returning host, the phone's menu, on scroll, the foot's job, the foot's door, two doors to one loop | answered whole (batch four); `chrome-wiring` (Opus, cut `b30445d9`) lands the four byte-changing picks (the bar hides going down and returns coming up, a Dashboard hint for a signed-in host, Start free always with the demo beside it when set, both nav doors to `/how-it-works`) and keeps the four as-today picks; round two `footer-close` (Sonnet) draws the footer against the closing CTA, his ask by name |
| `event-identity`, `event-type-pages` | round one of the event pages' visual identity from the ground up (the hero's theme, the second section, the arc, the cards, the proof, the ladder, the phone) and the direct picks (one template, four types, the host, the 2x2 grid, `PageHero`, the phone gap) | both answered whole (batches three and four); `events-wiring` (Opus, cut `b30445d9`) builds the hub and the four type pages to production on the seven directions plus the direct picks (a lit object per type carrying the demo's real code, the statement, the photograph turn into paper, the photograph card for all four, the door proof, the hero subhead on a 20 to 22 clamp site-wide, the phone half-and-half), judged on the alias; both boards retire with it; ASSETS rows 24 and 25 serve it |
...
His sitting continues on the desk (twenty-five boards unruled after batch four: `voice`, `glass`, `loose-ends`,
...
`reel-studio`, `host-curation`, `admin-triage`, `emails`, `help-center`, `export-flow`; then `site-chrome` and `profile-page` round two as they land). Five lanes run from batch four (three wiring lanes judged on the alias as they land, two round-two boards for the desk); `/how-it-works` with the home's stepper and every failure page are on the alias for his eye, five
