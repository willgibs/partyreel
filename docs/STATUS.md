...
| `heroes` | `privacy-hero` (the field as two spirals, faster, a trail) and `album-page` (round four at the home hero's pace, subtle, the live album beneath at 896 with its bottom faded, a pool of light) | integrated at `6b5ea1bf` (resumed on Sonnet after the kill): eight decisions, every option a real Frame at 1440 and 375; `album-page` on the desk; `privacy-hero` round one answered none (transcribed into its ledger from his note), round two on the desk from `image-trail` |
...
| `image-trail` | our own cursor-tracking image trail (the Codrops "Image Trail Effects" demo one as the reference, written from scratch: a photograph born on every stretch of cursor travel, sliding to the cursor and decaying behind it) with real homes on the marketing site, and `privacy-hero` round two on the same engine (tighter, faster, the trailing photographs decaying) | integrated at `dd44692e`; `image-trail` two of six answered (`d140`, `long` with three seconds suggested), the rest and `privacy-hero` round two on the desk; five questions carried on their recommendations (the keeper, the shy fade, the 404 as part of the home answer, `rush` for the pace) |
| `cursor-backdrop` | full-bleed photograph sections that switch with the cursor (demo six of the same resource) for the marketing site's UI-forward chapters in place of the aurora, breaking the strict dark/light alternation: where it sits, how it switches, how the copy stays legible, the phone | answered in full (`full-quality`, `plate`, `band`, `slide`, `insert` as a soft ruling, `scroll` at four or five steps) with the chapter-transition ruling; `backdrop-wiring` cut from it (Opus, :3131); the room-frames asset ask is row 20 |
...
His sitting opened on `privacy-hero` with none (2026-09-18, night); its first batch of the round's second sitting (build `93d00ff`) ruled `cursor-backdrop` whole and began `image-trail`. The desk derives the steps (`/design/lab?key=`): all eight boards of the round are integrated and his sitting is open
