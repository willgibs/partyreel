...
- **Media durability (ADR-0013).** Pillar A (orphan-sweep circuit-breaker) **shipped** (commit `6f151c5`): the cron's orphan sweep now deletes nothing + alerts if `media` is empty or the orphan set is pathological, so a DB fault can't wipe the un-backed-up R2 bucket. Pillar B (real-time CF Worker → Bucket-Locked 2nd R2 bucket, `workers/backup/`) is **DEPLOYED + DR-drill-verified** (2026-06-06: ~15 s replication, the 35-day lock blocks deletion, restore works, >100 MB multipart copy verified byte-identical); Pillar C (off-site `pg_dump` → R2 `db/` via `.github/workflows/db-backup.yml`) is **LIVE + restore-verified (2026-06-07)**: the daily dump lands in `partyreel-backup/db/` (object verified in the dashboard, IA + 35-day lock) and a logical dump restored cleanly into a throwaway Postgres 17 with every table's row count matching production (15 tables / 33 functions / 10 policies / 23 rows). Secrets are set (`SUPABASE_DB_URL` + `R2_BACKUP_*`); hardened with a post-upload byte-size verification (zero silent failures) + a Node-24 opt-in (ahead of GitHub's 2026-06-16 cutover). **All three durability pillars shipped.** Master plan: `.claude/plans/we-ve-recently-pushed-a-breezy-nest.md`. _Note: `media` is currently empty, so the breaker is protectively active pre-launch._
...
- **Recovery / "Recently deleted" initiative — Phases 1–5 of 6 SHIPPED + DEPLOYED + LIVE-VERIFIED on partyreel.com (2026-06-03 → 06-04).** Master plan:
  ...
  non-deleted events) instead of the physical `storage_used_bytes`, so **deleting frees cap room
...
- **Custom event link (slug) — SHIPPED + LIVE-VERIFIED on partyreel.com (Phases 1 + 2;
  ...
  **Live-verified (Chrome MCP, prod):** set on a Pro event (uppercase normalized, UI flips to the saved
  state); the guest page resolves `/e/<slug>` and the live gallery polls with the CANONICAL token (the
  R3 fix, confirmed via the actual poll request body `{"qr_token":"<canonical>"}`); change → old slug
...
- **Profile photos + display names (3-part build) — COMPLETE. Phases 1-3 (avatar upload · display-name
  ...
  **Phase 2** (display-name editor + dropping the email-prefix `handle_new_user` fallback + a one-time
