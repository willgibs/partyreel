...
| **The CI budget round** | **✅ 2026-09-15**: round four's twelve tracks ran the remote gate on every push (405 runs, about 1,500 GitHub Actions minutes on 09-14 and 09-15, 90 percent on `lp/*`) and spent the month's private-repo budget in two days; Will made the repo public for the interim. Policy now: CI is not the gate, the four local steps are; `ci.yml` runs on `main` and `launch-prep` pushes that touch code (docs-only pushes skipped, the Orchestrator's record commits say `[skip ci]`) and on PRs to `main`; an `lp/*` push runs it only on `[ci]` in the commit message. Vercel: `preview: true` no longer builds every push; an agent branch builds at `status: handed-off` or on `[preview]` (twelve boards building every push hit the 100-a-day cap at 23:31 on 09-14). **Yours**: swap the repo back to private when the Actions budget clears. |
