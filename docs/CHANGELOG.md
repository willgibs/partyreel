...
**Will's review pass, same day.** Six notes, all applied. The page moved onto the RESOURCE-PAGE
CHAPTER SHAPE he and the /about track had just settled (cinema hero, paper body, dark footer) with the
sheet riding inside the hero chapter rather than forming a mid-page block. Every section centered on
one rhythm, since the mixed left-aligned hero / full-bleed sheet / narrow body read as jarring, and
the close in particular had to stop being left-aligned or it stacked against the footer's own
left-aligned demo invitation. The hero's bare "Copy" button (which he had to press to discover what it
would copy) became Contact, with the support address inline-linked in the standfirst. Two stock photos
became four branded assets (the app icon, the real generated share card, and the QR in SVG + PNG,
generated by `scripts/build-press-qr.mjs` from the site URL so the download and the on-page island
cannot disagree); the kit went 6 files to 10, and losing third-party media collapsed a whole
provenance block into one line. Mono was pulled back to data only (indices, hex, prices, a year, file
sizes) with captions and the licence line in Inter. SVG/PNG became real bordered chips with hover,
focus and press states, the ink hex became a copy button showing its own value, and the fact sheet's
domain and address became links. Caught while in there: at 375px the caption left ~50px beside the
chips and truncated "Mark, dark chip" to "Mark…", so it stacks below sm.

**Unblocked for every agent:** `DESIGN_PREVIEW_KEY` had a `preview` row pinned to `gitBranch:
launch-prep`, which is why `/design` 404'd on every `lp/*` preview. Will supplied the key and an
