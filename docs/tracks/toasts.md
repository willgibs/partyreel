...
- His ask ("If no exploration has handled this already, I'd like to redesign our toasts"): none has; six boards decide a
  toast MOMENT each and keep their words. This board asks the SYSTEM on the real toasts (the six kinds sonner draws today
  from 253 calls), its first line the rule that halves them: IF THE CONTROL CAN SHOW IT, NO TOAST (`CopyShareLink` flips
  its icon and toasts today; a switch that saved itself needs no second announcement; Copy is drawn as the example).
  Five decisions: `where` (the foot of a phone and bottom-right of a desk, as today / the top under the bar / the foot
  on both), drawn WITH the floating action bar and the guest uploader's credit UI present, since the foot of a phone is
  where they live; `material` (the popover card, as today / ink on paper / the card with the house shadow only; glass
  is not an option and the board says why: glass is ruled over photographs, a toast sits over UI), `life` (4 s as today
  / by length / errors until dismissed), `stack` (expanded / collapsed as today / one at a time), `action` (an Undo or
  a door on the toast: always room for one / never / only on errors; the accessible pairing to draw is errors that
  persist with their action while success stays short). `motion` is not asked: bible 12 decides it (the house easing
  under 300 ms). Drawn on `guest-upload`'s "Sent, waiting for host approval", the host's bulk approve (with Undo), an
  export mint, a pricing refusal and an error, at 375 and 1440, both themes. Owns
  `sandbox/toasts/` and its registration lines; reads `ui/sonner.tsx`, `app/layout.tsx:89`, `globals.css:1052-1091`, the
  six boards' toast decisions (never edits). Registers at the head of `DESK_ORDER`; the Orchestrator moves it after
  `app-vocabulary` (a part under both shapes).
