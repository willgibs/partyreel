...
- The BUG first, REPRODUCED before it is called found (both symptoms on the rendered board at 375 and 1440: the
  colour at a photograph's edge, the horizontal edges inside a pure gradient): the root clips (`overflow-hidden`
  with its `rounded-full`; the `after:` ring survives) and the CHILDREN lose their own rounding (at 24 to 40 the rim
  is the image's own anti-aliased `rounded-full` edge over the painted root, which clipping the root alone does not
  remove); the image fills the disc (`size-full`, no inset); the board's `BigOrb` (`sandbox/seed-avatar/orb.tsx:186-195`)
  gets `box-border` and the photograph `inset-0`, so the 80 px photograph no longer overruns a 78 px box; the
  gradient's `background-size` and the board's fractional zoom checked as causes of the edges within. A contract
  test: a rendered `Avatar` at every size clips, its children carry no rounding of their own, and the image covers
  the disc with no gap.
- The generator moves home: `src/lib/avatar/gradient.ts` and its `@contract-for` test from the sandbox; the board
  re-exports from there and keeps drawing (it stays open for round two on `look`). `Avatar` gains `seed?: string`:
  when set, the root's `backgroundImage` is `background(orbFor(seed), "diagonal")` and the fallback drops `bg-muted`
  for a transparent ground with the initial in `inkFor(hue)`; the image always sits over the colour
  (`after-upload=under`); nothing moves (`motion=none`); the wheel, full colour, the initial at every size.
- The seed on every surface is `seedFor(profiles.id)`, a server-side SHA-256 of the id (one colour per person
  everywhere, and a client never receives an id it does not already hold: the guest page and the menu route keep
  `host_id` off the client on purpose): the user menu (`(app)/layout.tsx` has `user.id`),
  the account form (pass `userId` from `account/page.tsx`), the guest account menu (`/api/me/menu` returns `id`;
  `guest-header.tsx` forwards it), the guest list's chips and faces row and the owner sections (`ProfileCardItem.id`),
  the profile identity row's raw `<img>` folded onto `Avatar size="xl"` (80 px, a fourth size on the contract). The two
  "Hosted by" bylines (`event-experience.tsx:271`, `entry-modal.tsx:459`) are guest-wiring's files: this lane lands
  the API first and guest-wiring swaps them with a server-computed `seedFor(host_id)` after its sync (listed in both
  Handoffs). The duplicate paper `PILL` at `u/[slug]/page.tsx:66` goes onto the glass utility by whichever of this
  lane and glass-wiring lands second (one line, listed in both Handoffs).
- The variety guarantee (his note): the crowd test fed a thousand `crypto.randomUUID()`-shaped seeds fills every
  30-degree bucket and no bucket exceeds twice the mean; a concentration is a red test.
- Owns: `src/components/ui/avatar.tsx`, `src/lib/avatar/`, `src/app/(dev)/design/sandbox/seed-avatar/`,
  `src/components/social/`, `src/components/app/user-menu.tsx`, `src/components/app/account-avatar-form.tsx`,
  `src/components/guest/guest-account-menu.tsx`, `src/components/guest/guest-header.tsx`, `src/app/api/me/menu/`,
  `src/app/(app)/account/page.tsx` (by file: door-wiring adds its passkey card line there as its exception after this
  lane lands), `src/app/(guest)/u/`, `docs/systems/profiles-social.md` (the avatar's facts in place),
  `rules/component-notes.ts`'s Avatar `for` line (`pnpm design:rules`). Reads: the seed-avatar ledger, rulings.md.
- Red-team on the alias signed in as Will (his): the faces row on a profile, both menus, the account form, at 375: no
  grey disc, no visible edge, initials legible on every hue. Signed out in the pane: a public profile's faces row.
- His to overrule: the 80 px fourth size; the transparent fallback ground; the bucket bound.
