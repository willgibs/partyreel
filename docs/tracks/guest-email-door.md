...
- none raised mid-lane. Every open call in the brief was taken on its recommendation and is listed
  under "Calls his to overrule" below. No new one-way door was reached.

## System-doc edits (in place, owned facts only; the Orchestrator reads each by eye)

- `docs/systems/guest-flow.md` "Joining + identity": a new three-levels-of-trust block at the head of the
  section (the typed name and the word "Unverified"; the typed name plus an inert `pending_email`, whose
  public mark is IDENTICAL by design; the confirmed account), the optional field's shape and its
  `noValidate` reason, and the rule that nothing stores the address on the device. The join bullet now
  reads `{qr_token, display_name?, email?}` -> `{display_name, verified, email_attached}` with the
  "believe the row, not the form" note, and a new bullet for `POST /api/guests/email`.
- `docs/systems/guest-flow.md` "The ARRIVAL": one sentence on the name step's second, optional field
  (not a step, adds no step, `computeDoor` does not know it exists).
- `docs/systems/guest-flow.md` "Auth-aware header island": the third state's two menu states from the
  device flag, the add-email dialog, the empty confirm door and why, and "no Remove row this round".
- `docs/systems/auth-accounts.md` Invariants: one new bullet — a typed address at the guest door is
  inert, is accepted whether or not it belongs to a member, and the no-enumeration-oracle rule stands.

## Deferred (ROADMAP one-liners, bucket named)

- Guest experience: a "Remove your email" row in the guest's own menu (the RPC's detach arm and
  `attachGuestEmail(email: null)` both exist and are tested; the ruling held the ROW for the lab).
- Guest experience: the guest menu's add-email dialog, the offer card's prefill and the two menu labels
  are stand-ins for the identity-flows board Will asked for by name; their SHAPE is the lab's.

## Handoff (replaces the chat report)

- Board commit `f83e49ca` (the lane whole); sync-merge commit `6fcd9453` (`origin/launch-prep` had moved to
  `d554ef82` with `guest-email-server` merged at `e48ce3e1`); one commit after it for the DRY pass the wire
  made possible. Pushed to `origin/lp/guest-email-door`.
- Gates on the synced tree, each on its own exit code: `design:rules` ok (224 components, 1885 contracts
  from 117 contract tests) · specimens ok (140 on 101 entries) · `typecheck` ok · `lint` ok, exit 0,
  **9 warnings** (one fewer than the 10 baseline: an unused `useUploadQueue` import in
  `guest-upload.tsx`, a file this lane touched, was dropped) · `test` ok, **334 files / 3641 passed**,
  2 skipped · `build` ok, **257 pages** · `pnpm lab:smoke --base http://localhost:3134` ok, **422 checks,
  0 failing**. No board this round, so no `lab:demo`.
- Lane check: `git diff --name-only origin/launch-prep...HEAD` = the owned paths plus FIVE exceptions,
  each with its why:
  - `src/app/(dev)/design/rules/rules.generated.json` and `docs/design/library.md` — GENERATED by
    `pnpm design:rules`, which the gate requires; two new contract tests move them whatever else changes.
  - `src/app/(dev)/design/rules/component-notes.ts` — ONE new `for`/`unspecimened` entry for
    `add-email-dialog.tsx` plus two refreshed lines for the menu and the step (the round's rule: a new
    component earns its Library entry). Nobody else's `owns` names this file.
  - `src/components/shared/media-lightbox.test.tsx` (5 lines) and `src/components/social/guest-list.test.tsx`
    (1 line) — both pinned the mark's label as the literal regex `/name not verified/i`, which HIS re-ruling
    breaks. Each now imports `UNVERIFIED_LABEL` and reads it, so the next re-wording moves them for free.
    Neither file is in another lane's `owns`; the components themselves are untouched.
- The items, one line each:
  - `wire-client`: `joinEvent({email?})` sends the key only when one was typed, `JoinedGuest.emailAttached`
    reads the ROW's answer, `JoinRefusal` gains `email_invalid`, `attachGuestEmail` posts the token in the
    body; `checkGuestEmail` delegates to the server lane's `parseGuestEmail` and adds only "a blank field is
    an answer". Lands as the Library's `join.ts` contract, 23 pins.
  - `device-flag`: `GUEST_EMAIL_ATTACHED_PREFIX` + `setStoredEmailAttached` / `useStoredEmailAttached` in
    `use-stored-name.ts`; the ADDRESS is never written to storage, and `session-tokens.test.ts` pins the
    prefix out of the capability scan.
  - `name-step`: one compact "Email (optional)" field under the name in `join` mode alone, unfocused, never
    prefilled, its own refusal slot, the benefit line; `hold` and `profile` render none. His lede wired
    verbatim ("so the host knows who to thank"); `hostName` kept and ignored so the lab's fixtures compile.
  - `add-email-dialog` (NEW): the second chance from the guest's own menu — one field, the door's own
    promise, Save over `attachGuestEmail`, "Confirm it now instead" handing to the code door. Lands in the
    Library with its `new` badge and a six-pin contract test.
  - `menu-two-states`: "Unverified" + Add your email, or "Email not confirmed" + Confirm your email into a
    code door that opens EMPTY and says why. Pinned in `guest-header.test.tsx`.
  - `offer-prefill`: `hintEmail` threaded EventExperience -> GuestUpload -> SaveAccountPrompt ->
    SaveEventButton -> AccountDoor, from React state alone, so a reload asks again.
  - `mark-word`: `UNVERIFIED_LABEL = "Unverified"` and both popover sentences renamed to a CONFIRMED EMAIL
    rather than a verified name; a new `unverified-mark.test.tsx` (7 pins) is the Library entry.
  - `gate-line` + `entrySheetCopy`: both his sentences wired verbatim; `enter-event-prompt.tsx` reads the
    table, so it moved with it.
- ★ ONE DEFECT FOUND AND FIXED, which no brief predicted: a native `type="email"` field inside a form lets
  the BROWSER run constraint validation on submit — it blocked the submission with its own untranslatable
  bubble, so the door's refusal slot never filled AND THE NAME NEVER WENT EITHER. Caught by the pin, fixed
  with `noValidate` on both forms, and re-proved live (the refusal renders in place, nothing is sent).
- Calls his to overrule on the alias, one line each:
  - "Add your email" is a DIALOG from the menu rather than a step (the door's steps are held and ordered; a
    guest who reaches this is already inside the album).
  - The address is kept in `EventExperience` state for the visit alone, so a reload loses the offer card's
    prefill and the door asks again. The shared-phone rule beat the convenience.
  - The helper line is his words on both surfaces ("Come back to this album anytime, with every photo you
    add."); the dialog's title is "Add your email" and its quiet second choice reads "Confirm it now instead".
  - The confirm door's description when an address is attached: "Enter the email you added and we will send
    a code." (an empty field otherwise reads as broken).
  - The menu offers "Add your email" only where a session token exists to attach to; without one it keeps
    "Confirm your email", which mints its own row on the way through.
  - `guest-upload.tsx` lost a dead `useUploadQueue` import (a lint warning in a file this lane touched).
- The help articles this lane makes stale: none left stale — the two it owns were rewritten in place
  (`require-verified-emails-explained.mdx` gains a "What the mark means, and what you see" section naming
  the three levels and saying the host never sees an unconfirmed address; `what-guests-can-and-cant-see.mdx`
  says the same under "What guests never see"; both quote `<UiLabel>Unverified</UiLabel>`, which
  `help-ui-labels.test.ts` checks against shipped source). NOTE for `help-sync`: the claims lane owns
  `why-an-event-asks-for-your-email.mdx`, `how-guests-join-and-upload.mdx` and
  `save-an-event-and-find-your-uploads.mdx`, which describe the same door.
- Assets requested from Will: none.
- Proposed migrations / Worker / Vercel / Stripe / env changes: none.
- Live red-team on :3134 at 1440 and 375, against the real Supabase (disposable data only, on the
  disposable events `guest-view-menu QA`, `Personal Testing Throwaway`, `Gallery width (disposable)`,
  `Ghost check (disposable)`): the field renders under the name and unfocused; a junk address is refused in
  place with the name's own hint untouched and NO browser bubble and nothing sent; an empty field sends no
  `email` key at all; a typed address rides ONE post and the row stores it trimmed and lowercased with
  `pending_email_at` stamped, `verified_at`/`email`/`user_id` all null (checked by SQL); the device holds a
  boolean and NO address; the menu reads "Unverified" + Add your email, the dialog attaches for real (200)
  and the menu flips to "Email not confirmed" + Confirm your email WITHOUT a refresh; the confirm door
  opens empty with its own sentence; a verified-required event's name step renders NO field and its email
  step reads his gate line verbatim; the mark on a real tile reads "Unverified" with "Anyone can type a
  name. Will Test has not confirmed an email, so this is not proof of who they are."; 375 shows the whole
  step and the whole dialog with no overflow. NOT reachable from localhost, so the Orchestrator's on the
  alias: a real upload under an attached address, and the code round trip from the confirm door.
- Look at first: the name step at 375 with the new field (does the optional question read as optional?),
  then the guest menu's two states, then the mark's popover on a photograph.


## Record (one paragraph, past tense, at most eight lines; the Orchestrator fills the merge SHA)

Merged into `launch-prep` at `<sha>` (2026-09-22). Wave 1's door half of the guest identity round on
Will's ruling of 2026-09-22, which supersedes `address=none`: the name step grew ONE optional "Email
(optional)" field in names mode alone, unfocused and never prefilled, refused in place and sent in the
same post as the name; the client half of the wire (`joinEvent({email?})`, `JoinedGuest.emailAttached`
read from the row rather than the form, `email_invalid`, `attachGuestEmail`) delegates its parse to the
server lane's `parseGuestEmail`; a device flag records THAT an address exists and never which one, so the
guest's own menu is the one surface saying "Email not confirmed" and offering a new `add-email-dialog.tsx`
or an empty confirm door that says why it is empty, while the public mark reads "Unverified" everywhere
the one constant is read. His gate line and the email step's description wired verbatim. One defect the
brief did not predict was found by the pin and fixed: a native `type="email"` field let the BROWSER block
the submit with its own bubble, so the refusal never rendered and the name never went (`noValidate` on
both forms).
