...
| `reel-front` | integrated at `03e7e599` (handed off `fc858e91` after its resume); the album's head, seven asks on the desk | done | Sonnet, :3139 | nothing; the board waits for his desk review; the wiring after it |
...
| `reel-host` | integrated at `2ed05330` (handed off `baf8a6e0` after its resume); the host's side, six asks on the desk; the round's last board | done | Sonnet, :3135 | nothing; the board waits for his desk review; the wiring after it |
| `reel-story` | integrated at `8012e393` (handed off `5b15b40c` after its resume); the marketing story, seven asks on the desk | done | Sonnet, :3136 | nothing; the board waits for his desk review; the sweep takes its verdicts |
...
- **The lab key** is `DESIGN_PREVIEW_KEY` in `.env.local` (read it there, never echo it; ★ a local `next dev` logs every request URL WITH its `?key=`, so a dry run against a local server uses a dummy key (dev accepts any) and a dev log is never tailed unredacted: the Orchestrator printed the real key into its own transcript that way on 2026-09-22 and scrubbed every scratchpad copy; a check of a keyed alias page runs in headless Chrome with the key read from the file (`$S/alias-capture.mjs`), never the browser pane, whose URL would carry it; ★ pnpm prints the script line WITH its arguments into any log it is redirected to, so a log of `pnpm lab:demo --key` carries the key: grep such a log for its EXIT lines only, never tail or cat it, a lesson from 2026-09-18); `pnpm lab:demo
