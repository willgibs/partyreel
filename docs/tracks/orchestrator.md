...
| `help-sync`, `home-states`, `guest-verify`, `buttons-pairs`, `toasts`, `demo-doors` running (the weekly limit killed all six at 07:30 EDT on 2026-09-20; re-spawned into their worktrees at 12:10 EDT with what git showed); `type-sync` on the first freed seat; the eight exploration briefs shelved on Will's morning word (close the board, stack nothing) | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
