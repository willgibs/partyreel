...
| `buttons-wiring` | cut at the closing sitting's record: the icon one notch over its text (12/14, 14/16, 16/18), `sm` onto the ladder, the five `text-[15px]` overrides onto `cta`; `body-type` retires | running | Sonnet, :3131 | the `cta` icon at 18 his to overrule |
| `controls-home-wiring` | cut at the record: one shared `ViewMenu` (tile size, Sort, Filter with the Deleted lens) beside Add photos, Download and Select; `app-vocabulary` retires | running | Sonnet, :3132 | Sort real only if the client holds the whole list |
| `home-states-wiring` | cut at the record: the real collapse (three by tone, "N more"), the band order next-step, storage, events, Just arrived; `app-shape` retires | running | Sonnet, :3133 | the Print chip stays; his to overrule |
| `guest-chrome-wiring` | cut at the record: the row then the dock, the door's shell in the Sheet's posture on vaul, the own-tile mark and its filter, the guest View menu after controls-home lands; `guest-shape` retires | running | Opus, :3134 | the door's desk posture (a right panel) his to overrule |
| `guest-verify-r2` | cut at the record: round two on the identity shape whole (five decisions; his three cases the stage; the wall re-measured; the four round-one rulings held) | running | Opus, :3135 | the configured rate limits his read; the Handoff's recommendation read by the Orchestrator before the message |
| `overtaken-3` | cut at the record: the judgment pass for the batch's reach; asks reached by a held ruling badged as held, no line | running | Opus, :3136 | nothing |
| the closing sitting's six lanes cut 2026-09-20 at the record (the sixth batch's lanes all merged by 13:40 EDT); the wiring of `guest-verify` waits for round two; no NEW board (the eight briefs stay shelved) | integrate in the program's order (gate 66 onward); `controls-home-wiring` before `guest-chrome-wiring` when both are ready; the buttons lane's two exception files (`entry-modal.tsx`, `password-gate.tsx`) sit under the guest lane's prefix and its five retirements land with the two policy entries in one commit | running | Opus / Sonnet | his eye on the alias after each; the three rate-limit numbers his |
