...
| `reel-engine-video` | integrated at `31171495` (handed off `3183921b`); the window reader, the budget, the ladder, prepareFrame, the frameAt ring, the encoder seam, the harness | done | Opus, :3137 | ANNOUNCED for `reel-engine-live`: sync past this merge before handoff (the video asset's shape `ReelClip.video`/`ReelVideoSource.frameAt`, `prepareFrame` in `engine/video/prepare-frame.ts`, the harness's range route as the fixture precedent); the alias range fetch after Will's CORS command is the Orchestrator's check |
...
| `reel-host` | Lane 87, wave 2 of THE REEL ROUND, a NEW lab board: the host's side of the reel (where Style lives, the Show the reel row, where Play on a screen lives, the dashboard's line, a host's own cut added to the album, Review's interplay; six asks) | cut `0abb6459`; spawned Sonnet on :3135 at ~15:12 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3135 | `integrate.sh reel-host <head> reel-host <msgfile>` at its handoff, `record.py`; DESK_ORDER after `reel-cut` |
