...
| `first-event-wiring` | cut at the third batch's record: the first event's eight rulings (the big name field, the style step redesigned, the door at the cap, the print stock, the beat, the share sheet, the launch list, the live hub); `first-event` retires | running | Opus, :3131 | the review of the door's copy, the mini-modal kept, the legacy dialog retired, the stock's one design, the live mechanism his to overrule |
| `guest-upload-wiring` | cut at the third batch's record: the upload act's eight rulings (the intent sheet and the review step, the stack tile, the sweep and the unified arrival grammar, the waiting tile, the failure sheet, the terms line, the reading rung); `guest-upload` retires | running | Opus, :3132 | the review step, the terms line's words, 16 px, the retired toasts his to overrule |
| `overtaken-5` | cut at the third batch's record: the judgment pass for sixteen verdicts on the fifteen standing boards; the fifteen overrides closed, the spent hold recorded | running; MERGES FIRST (its test refuses a retired board's key) | Opus, :3133 | nothing |
