...
| `album-pages` | the manifest, links by id, the version and change-log migration, the store and routes | its migration applied (`album_version`; the rolled-back check held; advisors +2 `rls_enabled_no_policy` by design), types at `6063f1d8`; finishing (agent `ad4b8ad9c6a69ad06`) | Opus, :3137 | its routes and store on the new types |
| `identity-email` | the account's email change confirmed at both addresses with the copies following, the typed name surviving the magic link, the deletion scrub going forward and its nets; the backfill written, never applied | building (agent `a0048aa232de8bba2`) | Opus, :3135 | ends a turn with "migration ready at <sha>"; the dashboard's Secure email change and Change Email template follow its merge |
