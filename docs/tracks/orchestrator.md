...
| `demo-wiring`, `welcome-tour`, `album-controls`, `avatar-look`, `pricing-fit` running; `demo-doors`, batch five's four (`guest-verify` cut next), `help-sync`, the two follow-ups queued | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
