...
| `identity-fixes` | integrated at `c1199470` (handed off `5fd5fbe7`; one declared exception outside owns, the `component-notes.ts` entry for `sentry.ts`; `design:rules` regenerated) | done | Sonnet, :3131 | the red-team of its six fixes on the alias once it moves (the pane at 375: a queued file across the flip with the sheet staying until closed; the legacy token `ab7cc94d` naming its own row; a presign 403 arriving in Sentry tagged vercel-preview; the teaser's one number; a rename patching the credits; "Keep this photo"); then the door round's two lanes cut at the record |
| `door-steps` | the door round's wave 1, the guest side: the three-step held door (welcome, password, name, email, upload), the decision with a reason (`resolveGalleryDecision`, `resolveViewerDecision`, the `g3` ETag), the `pr_guest_<eventId>` cookie and its leave route, the lifted queue and the in-sheet upload step with the server-owned fail-open, the two download routes behind the decision, the marketing mock phones and the door's nine help articles | running: cut `8d83ec75`, spawned Opus on :3132 at ~21:11 EDT 2026-09-21 (the agent id is in the Orchestrator's pickup paragraph) | Opus, :3132 | `integrate.sh door-steps <head> none <msgfile>` at its handoff (gate 94 or 95), `record.py`; `reshape-guest-capture` (Lane 69, Sonnet, lab-only) generated and spawned at its merge; `[preview]` after the second production merge, the alias, the prune, the plan's Verification red-team |
| `upload-gate-host` | the door round's wave 1, the host side: the Require an upload to view switch (the first `ConfirmSwitch` to ask on the ON edge), the zod schema and the mutation's insert and patch lists, the composed guest-experience sentences, the new help article and the two it touches, the host-app, PRD and PRICING lines | running: cut `8d83ec75`, spawned Sonnet on :3133 at ~21:11 EDT 2026-09-21 (the agent id is in the pickup paragraph) | Sonnet, :3133 | `integrate.sh upload-gate-host <head> none <msgfile>` at its handoff, `record.py`; `[preview]` after the second production merge |
