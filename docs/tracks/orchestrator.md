...
| `reel-engine-video` | integrated at `31171495` (handed off `3183921b`); the window reader, the budget, the ladder, prepareFrame, the frameAt ring, the encoder seam, the harness | done | Opus, :3137 | ANNOUNCED for `reel-engine-live`: sync past this merge before handoff (the video asset's shape `ReelClip.video`/`ReelVideoSource.frameAt`, `prepareFrame` in `engine/video/prepare-frame.ts`, the harness's range route as the fixture precedent); THE CORS PRECONDITION LANDED (Will, 15:35 EDT 2026-09-22, `wrangler r2 bucket cors set` in the R2 API's rules shape: `range` allowed, `Content-Range`, `Accept-Ranges`, `Content-Length` exposed) and was VERIFIED from the alias origin in the pane: a range fetch of a presigned object answered 206 with `Content-Range: bytes 0-1023/294018` readable; the invariant is in uploads-and-r2.md and `client.ts` at `0829e713` |
| `reel-view` | integrated at `b063f1cc` (handed off `785f9797`); the reel's full-screen view, eight asks on the desk, first of the reel boards | done | Sonnet, :3138 | nothing; the board waits for his desk review; the wiring after it |
...
| `reel-screen` | integrated at `ac325953` (handed off `c060f326`); the venue screen, eight asks on a 1920 wall of its own, on the desk | done | Opus, :3131 | nothing; the board waits for his desk review; the wiring after it |
...
| `reel-story` | Lane 88, wave 2 of THE REEL ROUND, a NEW lab board: the marketing story of the reel (the thesis line, the /reel page's arc, the home's teaser and the hero film's role, the pricing rows' words, the how-it-works steps, the event pages' reel column, the help category's name; seven asks) | cut `0abb6459`; spawned Sonnet on :3136 at ~15:20 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3136 | `integrate.sh reel-story <head> reel-story <msgfile>` at its handoff, `record.py`; DESK_ORDER after `reel-host` |
