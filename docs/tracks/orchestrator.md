...
| `toasts`, `home-states` | his asks by name (batch five): the toast system, the home across host states | running (resumed 2026-09-20 12:10 EDT) | Sonnet | cut as seats free |
...
| `help-sync`, `home-states`, `toasts`, `demo-doors` running (the weekly limit killed all six at 07:30 EDT on 2026-09-20; re-spawned into their worktrees at 12:10 EDT with what git showed; `guest-verify` and `buttons-pairs` handed off since); `type-sync` cut on :3133 at the record; round twos on open boards and the wiring of rulings continue as before (his clarification, 12:20 EDT); no NEW board until the desk is closed: the eight exploration briefs stay shelved | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
