...
1. **The picks sweep** (Sonnet) after `library-lean` merges (Will, 2026-09-24: a pick is the best of what was drawn,
   never a rule; what ships is a working version). About 250 comment and lab lines in `src/` cite a pick as authority
   ("Will ruled", "law", "worn here rather than re-judged", "his pick"; re-grep at the cut): each keeps its WHY
   and drops the authority, and a line that is only authority goes; the desk's "ruling" vocabulary where it means
   his answers. Owns: the prefixes the grep finds, outside the held guest lane (whose own lines ride its sync
   message), with `globals.css` and `theme.css` released from this file.
2. **The design docs' pass** after `library-lean` merges (`design-system.md`, `marketing-content.md`, `host-app.md`), the same guideline and brief shape as `systems-lean` (`specs/systems-lean.json`).
3. **The reel round**, after Will's desk review on build 5 (the plan file's batch-1 and reel sections).
