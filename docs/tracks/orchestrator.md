...
| `verified-email-migration` | the identity reshape, wave 0: the expand migration (`require_verified_email` beside the old flag under a sync trigger, `guests.display_name` and `verified_at`, the forensics name, the six functions), the rolled-back check, the migration-text tests | cut 2026-09-21 14:50 EDT, running alone; applied, checked, types regenerated and integrated by the Orchestrator at its handoff, before wave 1 is cut | Opus, :3131 | the production read on `main` after the apply |
| `verified-email-server` | the identity reshape, wave 1: the join and rename routes, the 403 after a flip, the identity's one precedence rule, the guest list's union, the host card, the forensics capture | queued for wave 1 (cut from wave 0's record) | Opus, :3131 | the alias after wave 1 |
| `verified-email-guest` | the identity reshape, wave 1: the name step at the first Add, the credit's mark with its way out, the guest menu, the offer card on every sign-in path, the follow moment | queued for wave 1 | Opus, :3132 | the alias after wave 1 |
| `verified-email-host-copy` | the identity reshape, wave 1: the switch Require verified emails, the host's surfaces, the marketing, help, legal and doc sentences, the sweep for the word | queued for wave 1 | Sonnet, :3133 | the alias after wave 1 |
| `verified-email-lab` | the identity reshape, wave 1: `guest-verify` retired on his four answers; the desk pass across the fourteen standing boards | queued for wave 1; merges last of the wiring | Opus, :3134 | nothing |
...
| `reshape-viewer-curation` | the overtaken audit: `media-viewer` and `host-curation` reshaped (fourteen badged questions, the context folded in, `none` dropped, `opening` redrawn) | cut 2026-09-21 15:12 EDT, running | Opus, :3136 | the boards at 375 and 1440 |
| `reshape-studio-export` | the overtaken audit: `reel-studio` and `export-flow` reshaped (sixteen questions, five dead options dropped, the door redrawn, the stack option new) | cut 2026-09-21 15:12 EDT, running | Opus, :3137 | the boards at 375 and 1440 |
| `reshape-admin-help-emails` | the overtaken audit: `admin-triage`, `help-center`, `emails` reshaped (seventeen questions, eight dead options dropped) | cut 2026-09-21 15:12 EDT, running | Sonnet, :3138 | the boards at 375 and 1440 |
| `reshape-marketing-boards` | the overtaken audit: `site-chrome`, `profile-page`, `privacy-hero`, `album-motion`, `loose-ends`, `contact-page`, `press-page` reshaped (nineteen questions, `hover` dropped, two new concepts, two redraws) | cut 2026-09-21 15:12 EDT, running | Sonnet, :3139 | the boards at 375 and 1440 |
