...
| `demo-wiring`, `album-controls`, `avatar-look`, `pricing-fit` running; `guest-verify` and `buttons-pairs` cut next; `demo-doors`, `toasts`, `home-states`, `help-sync`, the two follow-ups queued | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
