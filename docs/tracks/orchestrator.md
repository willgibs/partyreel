...
| `third-batch-fixes` | the alias red-team's follow-up on the third batch: the failure sheet re-opening with a dismissed failure, the print card's link line overflowing the card, the sheet's flash on Send | cut 2026-09-21 12:25 EDT, running | Sonnet, :3131 | the red-team on the alias after its merge (the pane at 375: a refused file, Not now, then a clean run; the print page's nine cards measured on the alias hostname) |
