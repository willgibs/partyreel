...
| --- | --- | --- | --- | --- |
| `recheck-by-upload` | five boards current with tonight's rulings (identity-door, guest-capture, identity-profile, identity-claims, media-viewer.who) | running, cut at `8d1f66fd` | Sonnet, :3131 | integrate FIRST with `[preview]` (alias build 1: the desk current, the key `fiesta` live) |
