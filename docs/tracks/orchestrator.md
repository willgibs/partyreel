...
| `demo-wiring`, `guest-verify`, `buttons-pairs`, `toasts`, `home-states` running; `venue-screen` (the first exploration) cut next; `demo-doors`, `help-sync`, `type-sync` wait for demo-wiring | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
