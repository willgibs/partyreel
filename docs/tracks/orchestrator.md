...
| `door-steps` | integrated at `cc9e7da8` (handed off `ce3ce0d0`; five declared exceptions outside owns: a `for` line, two generated files, the Privacy version constant, two depicted counts; four Opus API drops survived by message) | done | Opus, :3132 | the plan's Verification red-team on the alias once it moves (the door at 375 signed out in both switch states, the fail-open, verified mode, the password event, the demo, the host, the mocks, the ETag and the two download routes by curl, the cookie heal, the leave path; Sentry); then the message to Will with the calls his to overrule and what to open first |
