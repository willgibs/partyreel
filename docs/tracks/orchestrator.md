...
| `demo-wiring`, `avatar-look`, `pricing-fit`, `guest-verify`, `buttons-pairs` running; `toasts` cut next; `demo-doors`, `home-states`, `help-sync`, the two follow-ups queued | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
