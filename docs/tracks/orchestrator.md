...
| `demo-wiring`, `avatar-look`, `guest-verify`, `buttons-pairs`, `toasts` running; `home-states` cut next; `demo-doors`, `help-sync`, `type-sync` queued | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
