...
| `help-sync`, `guest-verify`, `buttons-pairs`, `toasts`, `home-states` running; `demo-doors` cut next on :3136; `type-sync` waits for a seat; then the explorations (`venue-screen` first) | the queue after this wave (the plan file's "The queue after wave one") | running or queued | Opus / Sonnet | cut as seats free |
