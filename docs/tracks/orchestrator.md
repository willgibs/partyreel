...
| `upload-gate-host` | integrated at `95f8008f` (handed off `48725d49`; no exceptions) | done | Sonnet, :3133 | the switch walked on the alias as the host once it moves (the dialog on the ON edge, saved and read back, the summary both ways, the hint under closed uploads); the new help article at its slug |
