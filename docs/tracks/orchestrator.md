...
| `name-gate` | Lane 77, Will's ruling of 2026-09-22 (~11:52 EDT): a nameless account moves nowhere in the app but the welcome page; a nested (named) route group with one redirect, pinned; auth-accounts.md's line | running: cut `99a140b6`, committed `34970108`, spawned Sonnet on :3133 at ~11:55 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3133 | `integrate.sh name-gate <head> none <msgfile>` at its handoff (gate 104), `record.py` with `[preview]`, alias-ensure, the prune; the redteam on the alias: a nameless account cannot be minted without a code, so the pin and the build's route list stand |
| `identity-door` | Lane 78, a NEW lab board on Will's word (the identity flows): the door sheet with an email, the sign-in nudge, the gate's framing, the guest menu's two states, where Remove your email lives (five asks) | running: cut `50c03129`, committed `4addfb66`, spawned Sonnet on :3134 at ~12:06 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3134 | `integrate.sh identity-door <head> identity-door <msgfile>` at its handoff, `record.py`; the board on the desk for his review after his next project; a lab-only round verifies light (1440 and 375) |
| `identity-claims` | Lane 79, a NEW lab board on Will's word (the identity flows): the claim ticket's home, the pointer from the follow moment, the claim pass, the removal confirmation, what Finish does next (five asks) | running: cut `50c03129`, committed `4addfb66`, spawned Sonnet on :3135 at ~12:06 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3135 | `integrate.sh identity-claims <head> identity-claims <msgfile>` at its handoff, `record.py`; the board on the desk for his review after his next project; a lab-only round verifies light (1440 and 375) |
| `identity-profile` | Lane 80, a NEW lab board on Will's word (the identity flows): the profile setup with the wizard line folded in, choosing which attended events show, when the app invites the setup, what an empty page says (four asks) | running: cut `50c03129`, committed `4addfb66`, spawned Sonnet on :3136 at ~12:06 EDT 2026-09-22 (the agent id is in the pickup paragraph) | Sonnet, :3136 | `integrate.sh identity-profile <head> identity-profile <msgfile>` at its handoff, `record.py`; the board on the desk for his review after his next project; a lab-only round verifies light (1440 and 375) |
...
| `reel-view` | Lane 83, a NEW lab board, THE REEL ROUND: the reel's full-screen view (the chrome and its fade, the control set, the just-added beat, the tap, the posture, the hand pacing, the loop's turn, reduced motion; eight asks) | cut `17f17e57`; waiting for a seat (the identity lanes hand off first) | Sonnet, :3138 | `integrate.sh reel-view <head> reel-view <msgfile>` at its handoff, `record.py`; the six reel boards go directly below `media-viewer` in DESK_ORDER at the merges, `reel-view` first |
| `reel-front` | Lane 84, a NEW lab board, THE REEL ROUND: the album's head (the living tile, its verbs, the small states, the yours-first beat, the door's backdrop where the decision is already full, the head once uploads close, the host's hub card as the reel's face; seven asks; every tile option measured for its first-paint cost) | cut `17f17e57`; waiting for a seat | Sonnet, :3139 | `integrate.sh reel-front <head> reel-front <msgfile>` at its handoff, `record.py`; DESK_ORDER below `media-viewer` after `reel-view` |
| `reel-screen` | Lane 85, a NEW lab board, THE REEL ROUND: the venue screen at 1920 by 1080 (the QR's corner, the event's name, the caption, the wall pacing, the idle and empty states, the Start plate, Review on the wall, how the host opens it; eight asks) | cut `17f17e57`; waiting for a seat | Opus, :3131 | `integrate.sh reel-screen <head> reel-screen <msgfile>` at its handoff, `record.py`; DESK_ORDER after `reel-front` |
