...
# lp/guest-email-claims

**Goal.** Wave 1 of the guest identity round, cut by the Orchestrator at wave 0's record from Will's ruling of 2026-09-22 (rulings.md "guest identity: name only, unconfirmed email, verified account", every sentence his): the claim ticket as one plain card on the dashboard with its confirmation, the welcome page's name prefill, the attended-events default inverted, and the legal and help pages. No board, no new ruling. The lane's brief follows; read it end to end before the first edit.
...
- THE TICKET: a card at the head of the dashboard feed, rendered only when claimable rows exist: new `src/components/app/dashboard/claims-card.tsx` (client) fed by `getMyClaimableGuestRows()` in a new `src/lib/db/queries/claims.ts` (the server user client calling `list_guest_rows_by_email`; rows grouped by event: name, date when not withheld, the typed name(s), "N photos", the last-upload date) from `src/app/(app)/dashboard/page.tsx` (one insertion above the events feed; the empty teaser stays when there are no events AND no rows; with rows and no events the card sits above the teaser). Copy: "Photos waiting for you" / "Added at events with the email on this account, before it was confirmed." / per event "Claim" and "Not mine" / "Claim all" / "Finish". When any event is left unclaimed at Finish, a confirm dialog on `src/components/ui/dialog.tsx` (the `ConfirmSwitch` pattern in `confirm-switch.tsx`) names those events and asks "Permanently delete the N photos and videos added under your email at these M events?" with "Delete and finish" and "Go back"; nothing is written before the confirm; "Claim all" needs no confirm. Plain and small on purpose: the lab designs the ticket's real shape, the notification and the pointers after this round. New `src/app/(app)/dashboard/claims-actions.ts`: `finishClaimsAction({claimIds: string[] | null, disownIds: string[]})` → `getUser()`, then `rpc("claim_guest_rows_by_email", {p_event_ids: claimIds})` (null for "Claim all") then `rpc("disown_guest_rows_by_email", {p_event_ids: disownIds})` when non-empty, `revalidatePath("/dashboard")`, the toast "Added N photos to your account."
- THE NAME ON THE BY-EMAIL PATH: `dashboard/page.tsx` redirects a nameless profile to `/welcome` before the feed renders, so the card never meets a nameless profile; `src/app/(app)/welcome/page.tsx` prefills its name field from the most recent claimable row's name when the profile has none (one read over `getMyClaimableGuestRows()`), editable there; the RPC's nameless-profile arm stays as the belt. `src/lib/welcome.ts` untouched unless the prefill needs a line (then one line, listed).
- THE PROFILE DEFAULT: `src/components/social/attended-events-visibility.tsx` reads `shownOnProfile` and calls the server lane's renamed actions (announced in its Handoff: `showEventOnProfile` / `hideEventFromProfile` in `src/app/(app)/account/social-actions.ts`); the switch ON = shown, default OFF; the card's copy "Events you joined are private until you turn one on here. Turning one off never removes you from that event's own guest list (the host controls that)."; a new `attended-events-visibility.test.tsx` (ON inserts, default OFF). Verify the host's Recently deleted excludes uploader-removed items (`src/lib/db/queries/media.ts`); it does today; pin nothing there (not owned), note it in the Handoff.
- LEGAL: `src/lib/constants/legal-privacy.tsx` (Privacy to 1.5, the date): a Guest information row: "An email address you choose to add at an event that does not require one. It is stored unconfirmed and used only so you can claim your uploads from any device if you confirm it later. It is never shown to the host or to other guests. It stays until you confirm it and finish claiming: if someone used your address without you, confirm it and mark those events as not yours, and their uploads are removed and the address detached."; the local-storage line gains the per-event flag that records whether an email was added, never the address. `src/lib/constants/legal-terms.tsx` (the attribution line): "... uploads you made under a typed name from the same browser, or under an email you later confirm, can be claimed to your account; uploads someone else made using your address are removed when you mark that event as not yours." The legal version constants live in `src/lib/constants/legal.ts` (one line each, the door round's precedent: list it as your one exception outside owns).
- HELP: `content/help/why-an-event-asks-for-your-email.mdx` ("Rather not?" now says a names-mode event asks a name and offers an optional email, nothing sent; the name-before-email paragraph stands); `content/help/how-guests-join-and-upload.mdx` (the name step's optional email sentence); `content/help/save-an-event-and-find-your-uploads.mdx` (claiming by email from the dashboard card, "Not mine", the confirmation); `content/help/profiles-guest-lists-and-following.mdx` and `content/help/display-name-and-profile-photo.mdx` (attended events are off until turned on). Every quoted label shipped (`help-ui-labels.test.ts`).
- DOCS: `docs/systems/profiles-social.md` (the shown-events default, the claim card, one paragraph refined in place) and `docs/systems/host-app.md` (the dashboard card's line).
- Owns: `src/components/app/dashboard/claims-card.tsx`, `src/components/app/dashboard/claims-card.test.tsx`, `src/app/(app)/dashboard/claims-actions.ts`, `src/app/(app)/dashboard/page.tsx`, `src/app/(app)/welcome/page.tsx`, `src/lib/db/queries/claims.ts`, `src/lib/db/queries/claims.test.ts`, `src/components/social/attended-events-visibility.tsx`, `src/components/social/attended-events-visibility.test.tsx`, `src/lib/constants/legal-privacy.tsx`, `src/lib/constants/legal-terms.tsx`, `content/help/why-an-event-asks-for-your-email.mdx`, `content/help/how-guests-join-and-upload.mdx`, `content/help/save-an-event-and-find-your-uploads.mdx`, `content/help/profiles-guest-lists-and-following.mdx`, `content/help/display-name-and-profile-photo.mdx`, `docs/systems/profiles-social.md`, `docs/systems/host-app.md`. Reads, never edits: `src/lib/db/queries/social.ts` and `src/app/(app)/account/social-actions.ts` (the announced names), `src/lib/db/mutations/social.ts`, `src/lib/welcome.ts`, `src/app/(app)/account/actions.ts`, `src/components/ui/dialog.tsx`, `src/lib/db/types.ts`, `src/lib/constants/legal.ts` (the one-line exception), `docs/design/rulings.md`. Rewire the switch after the server merge; everything else needs no other lane.
- Tests: `claims-card.test.tsx` (grouping; Claim all sends null; Not mine; the confirmation only when something is unclaimed; Finish writes both lists); `claims.test.ts` (the query maps the RPC's rows); `attended-events-visibility.test.tsx`; the legal and help policies; `pnpm test` whole; the gate; `lab:smoke`. Your own red-team on :3135: the card with rows seeded by the Orchestrator's SQL on a disposable event (ask nothing; if none exist, drive the card with a mocked query in the test and say so), the confirmation, the welcome prefill, the switch both ways.
- His to overrule: the card's copy; the confirmation's sentence; the switch card's copy; the privacy and terms rows' words.
