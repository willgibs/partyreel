...
- **Commits, all pushed to `origin/lp/reel-guest-wiring`.** Work: `66a09fba` (the seam), `edac46b8` (the data through), `d88742fc` (the cut's seam), `e8ffbe21` (the guest side), `f2f91f1b` (the album's count label; guest-flow.md), `b6dc9f28` (the viewer's origin and moment, the card's reader, the address rule), `4800b595` (the idle wall's Start), `6dfce5f8` (every count on the guest album through `formatCount`), and the last work commit `747c7267` (his two rulings: the welcome comes first; every mood fills a landscape frame). Sync merges: `96442f54` (desk-refresh-standing, reel-refresh-cut), `60ea19a4` (media-viewer-wiring), `59d89808` (reel-refresh-host), `f6de87aa` (the record after build 5), `3b0f4c06` (his morning answers relayed, docs-sharpen cut).
- **Gates on the synced tree (`3b0f4c06`), each on its own exit code** (logs `/private/tmp/claude-501/-Users-gibby-local-ai-partyreel/401f4a77-be99-4a42-82f6-e5fac8e4a4c5/scratchpad/reel-guest-wiring/g7-*.log`): `pnpm design:rules` 0 (no artifact drift) · `collect-specimens.mjs` 0 (no drift) · `pnpm typecheck` 0 · `pnpm lint` 0 (7 warnings, none in a touched file) · `pnpm test` 0 (444 files, 4905 passed, 1 skipped) · `pnpm build` 0 · `pnpm lab:smoke --base http://localhost:3132` 0 (499 checks, 0 failing, `g7-smoke.log`).
- **Lane check:** `git diff --name-only origin/launch-prep...HEAD` is 65 paths plus this file, every one under `owns` but `src/lib/db/mutations/guest.ts`: two lines (`createMedia` takes `reelEligible` and passes `p_reel_eligible`), because it is the only door to `create_media` and the cut's seam must reach it. The rename `src/app/(guest)/e/[token]/opengraph-image.tsx` to `card/route.tsx` stays inside `owns`; nothing in the lab imports it.
- **Items:**
