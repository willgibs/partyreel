...
**Batch 2 (re-opened by message, 2026-09-25; his answers in `docs/reviews/reel-screen.json` round 2, `reel-host.json`, `album-columns.json`).** The screen plays in the window at once with the code on, under a glass pill ("Press anywhere to fill the screen"); the press takes fullscreen and the wake lock, and leaving fullscreen never pauses or releases (◇ reduced motion holds the first frame until the press; the sound stays silent). Show the code exists from 1024px up only (◇ the width). The view opens only when the reel is live: `?reel` under two is the plain album, and ◇ a screen under two, or dropping under two while it plays, shows the code alone (`IdleCode` without its Start button or line). The owner's extras inside the view: at 1024px and up a seventh icon, Play on a screen (`?reel=screen` in a new tab); the Style popover's footer "Only on this device, for now" with a "Set for everyone" pill calling `setReelDefaults` (reel-defaults-migration's) with the current look and hold; Close goes back where the host came from, else to the album; the owner's reel plays what guests see. Once the migration lane merges: the hold remembered per event, `readHoldSec(eventDefault)` falling back to `get_event_by_qr_token`'s `reel_hold_sec`, and the steps imported from `src/lib/reel/defaults`. "Clip" is the noun (`addClipToAlbum`, `addClip`, every "cut" a person reads). `BLEED` is 12px under 640 and 20px above. The album grid itself is `album-rows`'s. Hand off held, unmerged, for the stretch.
...
His rulings on this lane's two open questions (2026-09-24 morning):
- RULED (Will, 2026-09-24), the welcome comes first, everywhere: "Doesn't everyone without a guest name (unverified events) or confirmed email (verified events) get routed through the welcome flow? Then the welcome flow approves the guest and drops them off on the event page ... In my head, a host would login to a venue computer or send that laptop a link as guest to play the reel from event page after going through the welcome flow." Built in `747c7267`: a `?reel` or `?reel=screen` never opens under or over the welcome; for a visitor who still owes it the reel waits, then the link's reel opens; the owner is unchanged; no special case for the screen posture.
- RULED (Will, 2026-09-24), fill in landscape: every mood fills the frame edge to edge when the reel's composition is landscape (a laptop, an event screen), the default mood's letterbox bars dropped there; portrait unchanged. Built in `747c7267` (`fillLandscape`): the letterbox and Editorial's inset card are set aside in landscape, and a mismatched photograph stays whole on its own darkened blur instead of a flat colour (this lane's reading of "edge to edge" for a portrait shot on a landscape screen; his to overrule, the alternative being a crop to fill).

Batch 2, the ◇ calls, taken as the message wrote them (his to overrule):
- ◇ On a screen under reduced motion the window holds its first frame until the press (the press is the host's explicit act). Recommended: keep.
- ◇ Show the code exists from 1024px up. Recommended: keep (his "only desktop reel controls should have the QR toggle").
- ◇ A screen under two, and a screen whose album drops under two while it plays (or reloads there), shows the code and the address alone until the reel returns. Recommended: keep.

Batch 2, calls this lane took (his to overrule):
- Where the platform has no fullscreen (a phone's browser), the pill reads "Press anywhere to keep the screen awake" and goes once pressed. Recommended: keep; the alternative is no pill there.
- A phone's view (`?reel`) whose album drops under two while it plays returns to the album rather than showing the code (the code alone is a screen's state, as the ledger's `idle=code` note says). Recommended: keep.
- Before the press, the dock, the arrival chips and the code all work in the window; the press is taken from a tap on the picture or the pill, never from a dock control. Recommended: keep.
- The owner's Style footer says "Everyone sees this look" and offers no pill when the device already shows the event's defaults (the message's "Only on this device, for now" would not be true then). Recommended: keep.
- The owner's Close goes back whenever the tab has history (`history.length > 1`); a tab opened by Play on a screen has none and lands on the album. Recommended: keep.
- The payload's clip facts are renamed with the noun (`reel.clip`, `ClipFacts`, `clipFactsForTier`), not only `addClipToAlbum` and `addClip`, since the clip lane has not started and nothing else reads them; the engine's own "cut" (a hard cut, a cutaway) stays. Recommended: keep.
...
- `docs/systems/guest-flow.md`, for his two rulings: "The live reel" gains THE WELCOME COMES FIRST, EVERYWHERE (the door's `onPendingChange`, the reel waiting on it) and, under "The view", every mood filling a landscape frame (`fillLandscape`).
- For the Orchestrator, outside this lane's owns: `docs/systems/host-app.md`'s sentence "The motion-video engine (`engine/video/`, `lib/reel/live/`, `player-live.tsx`) is on the tree, but only the lab harnesses ... drive it: the production reel never passes a video source" goes stale at this merge, since the guest album's live reel drives all three in production (video windows under Include videos). Proposed: "... drive the guest album's live reel (guest-flow.md, "The live reel"), with the lab harnesses (`/design/lab/tools/reel-video`, `/design/lab/tools/reel-live`) beside it." (In host-app.md itself that name is the usual relative link; it is plain here because a link written in this file resolves under `docs/tracks/`, which the lab smoke crawls as a track.)
- `docs/systems/guest-flow.md`, batch 2: the two boxes' gutter (12px under 640), the host's default hold in the reel facts, the minimum's plain album and the owner's reel, the dock's desk-only code toggle, the owner's extras (Play on a screen, Set for everyone, Close back), the hold per event, the screen's window start and pill, the code alone for a screen under two, and "clip" throughout.
- For the Orchestrator, outside this lane's owns: three comments still name the deleted `components/guest/ghost-grid.tsx` / `<GhostGrid>` (`src/components/marketing/sections/features/shared/ghost-grid.tsx`, `src/lib/shared/use-in-view-sentinel.ts` and its test); the locked page draws `GhostRiver` now.
...
- Now: the FocusScope teardown flush that `entry-modal.test.tsx` now does in its own `afterEach` belongs in `vitest.setup.ts` for every component test that unmounts a Radix dialog (outside this lane).
- Now: `src/components/guest/yours-filter.ts` moves into `src/lib/guest/`, beside the other gallery arithmetic (`merge-gallery-items`, `reconcile-gallery-items`).
...
- **Commits, all pushed to `origin/lp/reel-guest-wiring`.** Work: `66a09fba` (the seam), `edac46b8` (the data through), `d88742fc` (the cut's seam), `e8ffbe21` (the guest side), `f2f91f1b` (the album's count label; guest-flow.md), `b6dc9f28` (the viewer's origin and moment, the card's reader, the address rule), `4800b595` (the idle wall's Start), `6dfce5f8` (every count on the guest album through `formatCount`), `747c7267` (his two rulings: the welcome comes first; every mood fills a landscape frame), `4cc2279b` (the lean round: comments keep reasons, not authority or history; the door test's teardown flake; `ghost-grid.tsx` deleted; the profile's label on `text-label`), then batch 2: `34653ce1` (the screen plays in the window, the code toggle at a desk, the view only when live, the owner's extras, clip, the phone gutter), `81888074` (the hold per event, the reel facts' default hold, the owner's Set for everyone footer) and the last work commit `92fe49eb` (on the migration: the event's hold default, the steps' one home, Set for everyone bound to `setReelDefaults`). Sync merges: `96442f54` (desk-refresh-standing, reel-refresh-cut), `60ea19a4` (media-viewer-wiring), `59d89808` (reel-refresh-host), `f6de87aa` (the record after build 5), `3b0f4c06` (his morning answers relayed, docs-sharpen cut), `d0283db4` (the new Library; the rules machinery gone), `72bef469` (refresh-guest and kit-streamline merged), `a3c4eb53` (reel-defaults-migration merged).
- **Gates on the synced tree (`92fe49eb`, launch-prep unmoved since `a3c4eb53`), each on its own exit code** (logs `/private/tmp/claude-501/-Users-gibby-local-ai-partyreel/401f4a77-be99-4a42-82f6-e5fac8e4a4c5/scratchpad/reel-guest-wiring/g10-*.log`): `pnpm typecheck` 0 · `pnpm lint` 0 (7 warnings, none in a touched file) · `pnpm test` 0 (439 files, 4764 passed) · `zsh scripts/build-lock.sh pnpm build` 0 · `pnpm lab:smoke --base http://localhost:3137` 0 (289 checks, 0 failing, `g10-smoke.log`).
- **Lane check:** `git diff --name-only origin/launch-prep...HEAD` is 173 paths plus this file (the comment sweep reaches every file in `owns` that carried authority or history), every one under `owns` but `src/lib/db/mutations/guest.ts`: two lines (`createMedia` takes `reelEligible` and passes `p_reel_eligible`), because it is the only door to `create_media` and the cut's seam must reach it. The rename `src/app/(guest)/e/[token]/opengraph-image.tsx` to `card/route.tsx` stays inside `owns`; nothing in the lab imports it.
- **Items:**
  ...
  13. Every count a guest reads goes through `formatCount` (en-US grouping, "1,145 photos & videos from 3 guests" on the scale probe, checked locally): the header, the locked page, the album's label, Yours (n) and its line, the teaser CTA, the door's lines, the capture flow, the dock's badge, the upload sheets, the closing card and the reel's arrival feed.
  14. The welcome comes first: EntryModal reports whether the door is owed (`onPendingChange`), the page holds `welcomePending` until the door says otherwise, and `LiveReel` opens an address's reel (`?reel`, `?reel=screen`, the idle wall) only once the visitor is through; the owner at once.
  15. Fill in landscape: `fillLandscape` (live reel only) drops the letterbox and the inset card and puts mismatched media on their own blur in a landscape composition; portrait untouched.
  16. The lean round: the `@contract-for` headers are gone from the lane's tests; comments across `owns` keep their reason and drop authority (no "bible N", no attributed rulings) and history (decision dates, phase and QA labels, board option ids), test titles that leaned on a label or told history now name what they check; stale notes corrected; `src/components/guest/ghost-grid.tsx` (imported by nothing) deleted; the profile's Events label on the `text-label` step.
  17. `entry-modal.test.tsx` unmounts and runs Radix FocusScope's `setTimeout(0)` before the document goes, so a loaded run no longer throws from `dispatchEvent` after teardown (a probe confirmed the unmount leaves the timer pending and the flush runs it).
  18. Batch 2, the screen: `?reel=screen` plays in the window at once with the code on, under a glass pill ("Press anywhere to fill the screen"); the press takes fullscreen and the wake lock (re-taken on every return to visible), leaving fullscreen never pauses or releases, the pill returns; without fullscreen the pill asks to keep the screen awake; reduced motion holds the first frame until the press.
  19. Batch 2, the code toggle from 1024px up only; the view only when the reel is live (a screen under two, or dropping under two while it plays, is the code alone; a phone's view under two returns to the album).
  20. Batch 2, the owner's extras: Play on a screen at 1024px and up (a new tab), the Style footer's Set for everyone (`setReelDefaults`, the device's look and hold; "Everyone sees this look" once they match), Close back where the host came from; the owner's reel is a guest's (approved, visible, reel-eligible).
  21. Batch 2, the hold per event, falling back to the host's `reel_hold_sec` (`resolveHoldSec`; NULL is the default), riding the reel facts as `holdSec` and the ETag (g7); the steps from `lib/reel/defaults.ts`.
  22. Batch 2, "clip" is the noun: `addClipToAlbum`, `addClip`, `ClipFacts`, `clipFactsForTier`, the payload's `reel.clip`, and every comment, title and doc line that meant the guest's own clip.
  23. Batch 2, `BLEED` is 12px under 640 and 20px above (the empty state's pull-out and the footer's hairline follow).
...
- **Look at first:**
  ...
  - Batch 2 locally (`captures/`): `b2-1440-screen-window.png` (the pill over a playing reel, the code on), `b2-1440-screen-pressed.png` (after the press: fullscreen, no pill), `b2-1440-screen-idle.png` (the code and the address alone), `b2-375-album.png` (the 12px gutter) and `b2-375-view.png` (no code toggle); the owner's extras and Set for everyone are pinned in `live-reel-view.test.tsx` ("the owner's Set for everyone", "the desk's extras") and `live-reel.test.tsx` ("a screen below two, and the owner's view"), and prove live only on the stretch's alias build (the owner signs in there).
  - His two rulings: `src/components/guest/reel/live-reel.test.tsx` ("the welcome comes first") and `src/components/guest/entry-modal-pending.test.tsx`; locally, a fresh browser on `?reel=screen` meets the welcome with no reel (`1440-welcome-first-reel-screen.png`) and gets the screen once through; landscape runs with a portrait photograph on its own blur (`1440-land-classic-03.png`, `-06.png`) and Editorial edge to edge (`1440-land-editorial-02.png`), all in the captures folder above.
