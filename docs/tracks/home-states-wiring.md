...
- Owns: `src/components/app/dashboard/` (every file; `filter-chips.tsx`, `trash-section.tsx`, `feed-section.tsx`,
  `empty-section-teaser.tsx` stay on disk with their props: the Library's compositions and the profile page import
  them), `src/app/(app)/dashboard/page.tsx`, `src/lib/dashboard/` (`next-step.ts` gains the pure fold rule: the top
  three by tone, the rest behind a count), `src/app/(dev)/design/sandbox/app-shape/` and the board's registration
  lines under the retirement exception, `docs/systems/host-app.md`'s "Dashboard landing" section. Reads: the
  app-shape ledger, rulings.md, the review sheet.
- The band order by his note (next-step, storage, events, Just arrived); the real collapse in `next-step-band.tsx`
  (three chips by tone, one "N more" chip that expands in place, reduced motion a plain swap; `aria-expanded`); the
  empty page checked against `wizard`'s own words; the one-event page unchanged. Tests: the fold rule (pure), the band
  (three then the rest on expand), `events-section.test.tsx` green, the page's order pinned in the existing
  dashboard test if one exists (else a `// @contract-for:` on the fold), `lab:smoke` whole, the gate. Red-team: the
  dashboard is Will's (signed in: `willg97` busy, `hi@willgibs` empty).
- Two notes: the pulse's own "Print the code" chip (an event dated tomorrow) stays, since `pulse` is "today's exact
  composition, unmodified" and his note refuses a share CARD, not the next-step rule (his to overrule); the head
  comments of `filter-chips.tsx` and `trash-section.tsx` that name the app-shape board as their importer are refreshed
  to name the Library compositions that still import them.
- His to overrule: the fold at three; "N more"'s wording; the storage line above the events; the Print chip.
