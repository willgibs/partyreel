...
- What this is: the reel round (rulings.md "the reel, reconceived", every sentence his; the plan `~/.claude/plans/great-work-however-1-dapper-twilight.md` "The concept, in one read" and section F). The reel is now the event's: a live, looping montage of everything the album shows, alive from the third item, faster in the hand than on a wall, its look the host's mood with a viewer's own switch. DECIDED, NOT ASKED (his rulings): the reel exists and loops; the host's mood is the default and a viewer switches looks on their own device; the player's chrome fades when the pointer rests and returns when it moves; "Include videos" is a toggle in the play controls; hide and Review are the only curation; no music; no download of the live reel (a cut is the download); the eight moods roll and the six treatments stay in cuts. A board is a CATALOG to select from (bible 22): every option a working picture, the same world (the wedding album the `media-viewer` board uses, at 1440 and 375), every reel frame drawn by the REAL engine over the fixture clips (the retired board's `stills.tsx` and `room.tsx` in git show how; `CanvasReelPlayer` with a `frame` prop scrub-locks a frame); every ask carries its context as a lede of at most 240 characters; a minute each for him. The retired `reel-studio` board's files are in git, not on disk: `git show 90f29be4:src/app/(dev)/design/sandbox/reel-studio/<file>` (`stills.tsx`, `room.tsx`, `pickers.tsx`, `surfaces.tsx`, `fixtures.ts`, `spec.ts`) shows how every reel frame was drawn by the real engine over fixture clips.
- ASKS (eight): `chrome` (what the view's chrome is and how it fades: a full-bleed picture with nothing until the pointer moves; a thin bar that fades; controls always on at the foot); `controls` (the control set and its order: play/pause, Include videos, Style, Add yours, Make your own, Close, drawn as three arrangements); `arrival` (the "just added by Theo" beat when a fresh item enters: a caption for one hold; a corner chip; nothing); `tap` (what a tap on the picture does: opens that item in the lightbox; pauses; nothing); `posture` (portrait on a phone and landscape at a laptop as one composition following the viewport, or the reel's own aspect letterboxed); `pacing` (three hand holds to feel, drawn as three running takes; the exact seconds are the knob); `loop` (how a new loop announces itself: nothing, the loop simply continues with a new take; a breath; a title card); `reduced` (what reduced motion starts on: paused with controls; the first frame with a play mark; the reel at a slower pace). Name the standing boards' nearest open asks and ask nothing they ask: `media-viewer.link` and `media-viewer.video` are badged by this ruling in `overtaken.ts` and remain theirs.
- Build from the kit (`src/components/lab/`; the app-side boards are the precedents; `src/components/guest/guest-reel-overlay.tsx` and `src/lib/reel/engine/player.tsx` are the truth for the shipped option, never edited); register in `registry.ts`, `boards.ts` and `touchpoints.ts` (your own lines only; a NEW board registers at the HEAD of `DESK_ORDER`; the Orchestrator moves the six reel boards below `media-viewer` at the merges); no em-dash; every string inside the registry test's limits.
- Owns: `src/app/(dev)/design/sandbox/reel-view/`. Reads, never edits: `src/components/guest/guest-reel-overlay.tsx`, `src/lib/reel/engine/`, `src/app/(dev)/design/sandbox/media-viewer/`, `src/app/(dev)/design/sandbox/gallery-fixtures.ts`, `src/components/lab/`, `src/app/(dev)/design/touchpoints.ts`, `src/app/(dev)/design/sandbox/overtaken.ts`, `docs/design/rulings.md`.
- Tests: the registry tests; `lab:smoke` whole; `pnpm lab:demo --board reel-view --base http://localhost:3138`; the gate with every exit code.
