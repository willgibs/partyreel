...
- His ask by name: "worth more dashboard explorations from here to figure out what feels best across all host states
  (from a new empty host to a first-event-just-created host to a busy host)". Round two on the same board id
  (`round.n: 2`), ONE decision `states` re-asked three times or THREE decisions (`empty`, `first`, `busy`), each drawing the
  ruled pulse under two or three shapes for that state (empty: the wizard's door alone / the pulse's bands as ghosts / a
  guided first screen; first event just created: the code and the share door first / the empty album's promise / the
  pulse with one event; busy: as ruled / the queues collapsed / the events first); the criterion for `empty` is his own
  ("The only goal of empty state is to get to feel polished while getting the user to having one event"). Drawn on
  `home-wiring`'s composition
  where it exists at cut time, else on the round-one drawings, at 1440 and 375. Round one's eight ruled asks named as
  ruled in the RULINGS row (and the row's `variants` corrected to eight). Owns `sandbox/app-shape/` and its lines.
