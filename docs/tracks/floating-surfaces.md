...
merged: "8779f561"      # the branch head merged into launch-prep
cut_round_5: "c473707"
...
**This board.** Five asks. `card | glass | command` are three directions for the whole floating layer
(menus, popovers, dialogs, sheets): label each by what it changes (the anatomy, the material, the
model) and say it in `context`; the `direction` control mirrors the ask (same ids) so wire `control`.
`sharp | nested | round`, `one-clock | by-frequency` and `today | shadow | follow-light` are the
corner, the entrance and the light in dark: each label in words, and where the matching dock control
carries an extra `off`, either drop `off` from the control or leave `control` unset. "Rule 12 against
rule 15" and "the palette board's dark ramps" need one clause of gloss each.

**Verify on.** The gate (`pnpm typecheck && pnpm lint && pnpm test && pnpm build`, each green); the board
at 1440 and 375 on a local `pnpm dev` (`/design/lab/floating-surfaces`), reduced motion honoured, every evidence
section showing the options' words; the desk's session on this board (`/design/lab?session=floating-surfaces.direction`)
read cold, as a stranger; `pnpm lab:review --dry 'review floating-surfaces r5: <ask>=<option id>'` accepting one
...
**Will's notes on this board, verbatim.** "This one actually has a great example of how the fixed/sticky
configurator works, good inspo for the rest of the lab pages that need that pagewide GUI always available."
"iframes also mess up the reviewability of these." "Overall, I don't really love our app design in general,
dropdown/nested menus included. The app is functionally great, but UI design lags far behind the design
work we've been doing for the marketing site. This is a global complaint across anything that touches the
app surfaces, and we'll need a lot of dedicated agent rounds to handle that once marketing is complete.
However, in the meantime, I prefer to use this track to explore new dropdown menu designs and variants of
our existing floating surfaces, rather than nailing the exact details of the current options. Any UI that
touches App in an active lab track may be worked on before the dedicated app agents get to it later."

**Round 4 (the goal).** The board changes purpose: from settling today's primitives to exploring what the
floating layer should become. (1) **New designs, from the ground up.** If this product had no menus, no
popovers, no sheets, what would the perfect floating layer be for a host on a laptop and a guest on a
phone? Show at least three genuinely different directions for the dropdown and nested menu (the header's
mega-menu, the event's actions menu, the account menu, a nested submenu), and variants for the other
surfaces (the dialog, the sheet, the drawer, the tooltip, the toast, the select), each a real working
component in the lab (compose on the primitives or write new ones under your lane), on the real grounds at
1:1, with its motion. A direction may change the material (glass, a lit edge, paper), the anatomy (a header
row, an icon rail, grouped sections, a search), the motion (unfold, slide, morph from the trigger), or the
model (a command palette in place of nested menus, a bottom sheet in place of a popover on the phone). (2)
**Frames at true pixels.** Your iframes render at 1:1 (never scaled), or the specimens leave the iframes
where a portal is not needed. (3) **The dock** stays your sticky bar, now the shell's `BoardDock` (adopt it,
or keep yours if it does more, and say which). (4) Keep round three's answer block for today's primitives
