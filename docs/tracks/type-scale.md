...
- Head: this handoff commit, sitting on `9814802` (the sync merge) over `b44a6aa` and `77a1e5c`; all
  pushed. Board at `/design/c/type-scale?key=`.
- ★ **The preview alias serves ROUND TWO, and no push of mine will change that today.** The project
  is at Vercel's ceiling of 100 deployments a day: the API returns exactly 100 rows for the trailing
  24 hours and the oldest of them is 16:24 on 2026-09-14, so no slot frees until about 16:24 on the
  15th. Three pushes on this branch produced ZERO deployments, and the newest `lp/type-scale` build
  on the project is round two's `2aa727b`. Same cause the floating-surfaces track recorded at
  `943473b`. **So walk round three locally**: `pnpm dev` in this worktree, then
  `http://localhost:<port>/design/c/type-scale?key=`; the lab is in no allow-list, so localhost
  renders the tip exactly, "Apply to the site" included. For the Orchestrator: force a redeploy of
  the tip once a slot exists (`POST /v13/deployments`, `gitSource {type: github, repoId: 1252816746,
  ref: lp/type-scale, sha: <tip>}`) and confirm READY. **The `launch-prep` alias is further behind
  still: it serves round ONE of this board** (eleven stages, no Apply bar), which is the same
  ceiling, so round two was never walkable there either.
...
- Gates on the synced tree: typecheck ok, lint ok (0 errors, 6 pre-existing warnings, none in this
  lane), test ok (1732 in 193 files; `ladders.test.ts` is 75 cases, 13 of them round three's), build
  ok (248 pages).
...
- **For the Orchestrator, three things outside this lane (the first two carry over from round two):**
  ...
  3. The wiring round needs the three one-line HOOKS round two asked for (`data-slot="page-heading"`
...
root 404 mount no design island, so no candidate ever reached them. Lab only; no production byte
changed.
