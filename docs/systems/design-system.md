...
**The working rules are the bible on `/design/library/rules`** (10 rules, Will's; source
...
- **`--brand` is an ALIAS of `--primary`** (ink). Never reintroduce a brand hue: photography supplies
  all colour and the accent stays off (bible 6); marketing may carry light of its own (the Aurora,
...
| # | Law | What it kills |
| --- | --- | --- |
| 1 | **Source.** Name the lamp: the object emitting, or the PLACE it lights. | Decorative glow on section edges, cards, borders, "anything that could use some life" |
...
**This sits over both law sets:** a lamp may light a section without media, because a lamp needs a
...
**The lamp set is LIGHT, never UI.** Never a text, border, background, state or brand colour:
...
its entrance and its air as much as from its type. Marketing travels further than the app (bible 8), only
...
LABEL**, with anything more specific in the eyebrow. `whitespace-nowrap` is load-bearing under
...
**TWO FACES, AND ONLY TWO** (bible 8): Inter for everything a person reads, Urbanist for what the page
...
**Anything drawn AROUND an object takes the object's radius, never a literal:** a ring, glow
...
- **A surface lying flat takes neither shadow, in either mode.** A shadow on a flat dark ground is a
  smudge and on a flat light one it is a fifth technique; a card, a field, a segmented
...
- **The hook sits on the box that owns the radius**, and the radius is inherited, never typed (a
  hand-typed radius on a wrapper draws a second arc). `event-card.tsx` is the standing trap:
...
- **The edges are INSET SHADOWS, never a border**: a border changes the pane's size, while an inset
  shadow draws on the padding box at the surface's own radius (a concentric arc for free).
...
- ★ **GLASS IS MEDIA CHROME, NEVER A POPOVER.** `floating-layer.ts`'s refusal of a backdrop filter on a
  ...
  ring. Two tests fence it from both sides.
...
**The floating-layer contract: one radius, one entrance, one light, as a MODULE rather than a sentence.**
...
- **The corner**: a 12px panel around 8px rows. `floatingCorner` is `rounded-float`; `floatingRow` is
  `calc(var(--radius-float) - 4px)`, DERIVED from the panel's own 4px padding (its inner = outer
  minus the gap), so moving a panel's padding moves its rows' corner.
- **The entrance**: one LANGUAGE per kind, the clock inside it chosen by frequency (the contract and
  bible 5 agree that way). `floatingEntrance` is the anchored one (a fade, a hair of scale, 8px of travel
  ...
  affordance). Every exit is faster than its entrance and nothing exceeds bible 5's 300ms.
...
**THE ARRIVAL-DEFAULT CONTRACT (bible 5): the VISIBLE state is the default; the hidden state belongs to
...
Copy rules: plain language, no em-dashes, no internals; the wording itself is open like every
other line on the site (bible 10), so a round that improves an error message is doing its job.
...
takes over, and walks alone below 640px. Nothing is laid over a photograph (bible 6): the words punch a
