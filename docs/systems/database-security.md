...
- **5 anon capability RPCs (lint `0028`, SECURITY DEFINER, executable by `anon` — by design, DO NOT
  ...
  **RETURNS TABLE IS the allow-list** (8 keys, pinned by [`guest-reel-contract.test.ts`](../../src/lib/reel/guest-reel-contract.test.ts)
  ...
  `allow_anonymous_uploads OR a signed-in viewer` (any `auth.uid()`, where the album's gate asks for a CONFIRMED
  email, so an unconfirmed signed-in viewer passes here while the album holds them at the teaser), mirroring
  `resolveGalleryDecision` in intent (an album that
