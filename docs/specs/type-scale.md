...
> **ROLE:** the `type-scale` exploration's proposal after five rounds (the review wave, 2026-09-14 to 15;
> round 4 on Will's review notes, merged `d762841f`; round 5 the Library x Lab migration), kept here so it
> outlives the track manifest. **NOT LAW** until Will rules; the board is `/design/lab/type-scale` while it
> stands, and every paste below is generated by the board's own code, never typed by hand. The record of each
> round is the CHANGELOG's review-wave entry.
>
> **The asks live in the board's spec, not here** (the migration wave, 2026-09-15):
> `src/app/(dev)/design/sandbox/type-scale/spec.ts` carries the question, the verdict, the four one-word calls
> with their options and the case for each, the candidates and the departures, and the template, the desk and
> the review ledger all read that one array. A second copy of them here is exactly the drift the spec ended,
> so the block that used to sit at the top of this file is gone.

**Round five.** **The board onto the kit's template.** Two files on the kit now: a `spec.ts` that is pure data
and a `board.tsx` that is the evidence per declared section. Nothing about the argument moved: the four
ladders, the pair, the token table and every number are round four's to the byte. What moved is the reading
order (the verdict and the four calls are the first screen), the comparison's shape (the two glance tables are
the kit's `SelectTable`, turned on their side so a ladder is a row read across and the row is the control that
picks it), and the frames (the kit's `Frame`, with the settle expressed as CSS rather than a MutationObserver
per frame). Two things the kit bought back: every hand-computed stage height is gone, so the ground ends where
the block does and each step's `where` line prints at 375 again, and the law stage's hand-estimated line count
is measured with `useLineCount` off the rendered text and printed. Lab only; no production byte changed.
