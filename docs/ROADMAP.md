...
The lab and the kit:
- Marketing type: the flat `text-3xl` FIGURES (the pricing teaser's and the calculator's price, /qr's pull quote) are off the ladder on purpose and fold in with whatever the sub-head tier becomes.
...
- **Generated media: one Higgsfield month** (Will, 2026-09-17, killing the media kit for it). Every photograph, and every clip and the hero film if it takes them on, is generated rather than shot or licensed, inside ONE paid month once more of the site is shaped, and lands before launch: the 12 stand-in stills sit in the chrome of every marketing page and in the blog's OG cards and RSS feed. Before buying, sweep `docs/ASSETS.md` for every photographic ask (rows 1 to 4, 9, 12 and 14 at least) so nothing needs a second month; the brief is written fresh from the site as it then stands (the old call sheet was deleted, "start blank"). Run it question-first: the look as round one (a Soul 2.0 moodboard), then each slot's frame staged `after` it. At 2026-09-17 prices, re-checked at the start: $19 / $59 / $129 a month for 270 / 1,200 / 3,000 credits; a Soul 2.0 image costs 0.12 credits, so video is what decides the tier; Plus's 5,000 free images and 7 days of unlimited Kling are website-only, and the MCP (`mcp.higgsfield.ai/mcp`) spends credits. Outputs stay ours to use after cancelling, but they are not exclusive and Higgsfield may train on them. Open at the start: whether it takes on video, and whether EU AI Act Article 50 (in force 2026-08-02) wants a disclosure line on a generated party. No row, field or test tracks where an image came from (Will's ruling).
