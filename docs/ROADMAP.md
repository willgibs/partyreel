...
- From `guest-verify` (2026-09-20, the lab; Will's read): the project's CONFIGURED Supabase Auth rate limits (Dashboard, Authentication, Rate Limits) recorded beside the documented defaults in `auth-accounts.md`; the board draws the defaults and says so; only the send cap can be raised, and the real number decides how bad an `outage` is at 120 phones.
