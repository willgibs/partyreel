...
- Guest: a seed that genuinely fails (a read error, not a refusal) takes the whole guest page to the error screen; an album-level boundary with a retry would keep the header, the door and the upload working while only the album says it could not load (from `owner-album`).
