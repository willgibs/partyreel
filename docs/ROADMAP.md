...
- From `event-identity` (2026-09-19, the lab; ruled `door`, so ASSETS row 26 is withdrawn): a per-type highlight render stays an idea (a type page's reel and the frames beside it would be that type's); `AlbumStream` takes one fixed photograph set, so the falling hero cannot be themed per event type without a frames prop (the cost `hero-theme=arrival` was drawn carrying).
- From `event-type-pages` (2026-09-19, the lab): the reunion post carries the `parties` audience tag while its subject is a trip and `blog-tags.ts` has no `trips` tag; three posts carry an audience tag with no link into a type page; the mega panel and the footer hand-write a third description per type beside `EVENT_TYPES.teaser` (consolidate in a lane that owns `marketing-nav.ts`; `events-wiring` gives `events.ts` the per-type media ids); `src/lib/constants/events.ts` sits outside the content policy's claim scan; the home teaser's "KNOWN MANIFEST GAP" comment is stale (the artifacts solved it for the type pages); `/events` has no `opengraph-image.tsx` while every type page does; `marketing-content.md`'s `/events` line names three deleted files (`EVENT_PRESENTATION`, `eventFrame()`, `event-frame-cards.tsx`); `events-wiring` (2026-09-19) takes the stale comment, the hub's OG image and the doc line.
...
- From `site-chrome` (2026-09-19, the lab): the footer's Product column hard-codes FAQ to `/#faq`, an anchor only the home has, so the row leaves `/pricing` (which has its own); the nav labels sit at `text-muted-foreground` under the transparent bar over a bright cinema hero (the balloons frame at 1440) and are hard to read before the glass arrives; `logo.tsx`'s `markOnly` branch is a placeholder tile with no caller (it goes the day the v1 icon lands, ASSETS row 19).
- From `profile-page` (2026-09-19, the lab; `profile-wiring` takes the identity row's 375 squeeze, the missing `loading.tsx` and the `(guest)` 404): three hand-rolled toggles do one job (`FollowButton`, the menu's block, the Connections card's buttons: one control, one contract); the "Someone followed you" switch promises mail no code path sends; the profile overflow opens over the person's own name at 375. Major overhauls: the follow graph has no consumer worth the graph ("Following" is one dashboard chip and nothing else reads a follow).
