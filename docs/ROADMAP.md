...
- From `event-identity` (2026-09-19, the lab): a per-type highlight render, so a type page's reel and the frames beside it are that type's (the `flanked` proof flanks a wedding with the shared render's DJ, crowd and confetti clips); `AlbumStream` takes one fixed photograph set, so the falling hero cannot be themed per event type without a frames prop (the cost `hero-theme=arrival` is drawn carrying).
- From `event-type-pages` (2026-09-19, the lab): the reunion post carries the `parties` audience tag while its subject is a trip and `blog-tags.ts` has no `trips` tag; three posts carry an audience tag with no link into a type page; the mega panel and the footer hand-write a third description per type beside `EVENT_TYPES.teaser` (consolidate after `site-chrome` and ONE PAGE OR FOUR land); `src/lib/constants/events.ts` sits outside the content policy's claim scan; the home teaser's "KNOWN MANIFEST GAP" comment is stale (the artifacts solved it for the type pages); `/events` has no `opengraph-image.tsx` while every type page does; `marketing-content.md`'s `/events` line names three deleted files (`EVENT_PRESENTATION`, `eventFrame()`, `event-frame-cards.tsx`).
...
- From `profile-page` (2026-09-19, the lab): `/u/[slug]`'s identity block (`flex flex-wrap items-center gap-5`) puts the avatar, the name column and the actions on one row, leaving the column about 90 px at 375 so "Joined June 2026" wraps to two lines (the board carries the fix below `sm`: `max-sm:basis-[calc(100%-6.25rem)]` on the column, `max-sm:w-full` on the actions); three hand-rolled toggles do one job (`FollowButton`, the menu's block, the Connections card's buttons: one control, one contract); the "Someone followed you" switch promises mail no code path sends; no `loading.tsx` under `(guest)` while `/u/[slug]` awaits an RPC and presigns; the profile overflow opens over the person's own name at 375. Major overhauls: the follow graph has no consumer worth the graph ("Following" is one dashboard chip and nothing else reads a follow).
