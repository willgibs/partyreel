...
- The lab, before the next round begins (Will, 2026-09-17: the stepped, form-based review "has been proven as an incredible resource"; once the current lab work clears, the lab system is upgraded again first): the kit findings below are the start of that list, plus one from his third batch: the desk cannot tell a reviewer that the page in front of them is a round behind the tree (he reviewed round seven on the alias while the tree held round eight; a build stamp on the board beside the round, or the desk refusing to compose a line for a round the ledger has closed). From his fourth batch: a winner's step puts the stage a screen and a half below the cards it answers to, so a press at the top changes something out of sight (a pinned, smaller stage beside the gallery, or the pressed card scrolling the stage into view); `lab:demo` reports corner-only options as the same picture, which is true at 1440 and is itself the finding (a corner family is judged on the phone-width cards, the stage shows the gap and the reflow); `board-state.tsx` still calls `history.replaceState(null, ...)` and drops the state object `step.tsx` preserves; a step has no dock, so "Reload frames" is unreachable there.
