...
- From `guest-verify-r2` (2026-09-20, the lab): the project's CONFIGURED Auth rate limits (Dashboard, Authentication, Rate Limits) are Will's read; the board draws Supabase's current documented defaults (verify 360 an hour per IP with bursts of 30; otp 360 an hour project-wide; emails 30 an hour on custom SMTP, raisable) labelled as such, and his numbers swap in through `call:numbers` without a redraw.
