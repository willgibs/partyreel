...
- From `avatar-look` (2026-09-20, the lab): measured with real compositing at the exact centre pixel, EVERY avatar look including the wired diagonal misses the letter floor on some seeds (the diagonal on 24 percent of a thousand, worst 4.13:1; `mesh` on none), a bar stricter than `gradient.test.ts` holds; the disc's ring border mitigates it in practice; the winner's wiring lands the stricter measurement in the production contract or names why not.
