...
- App boards Will asked for on 2026-09-19, cut on the next free seats: `gallery-controls` (a tile-size control "within/around our filter/sort/controls"; neither page has sort or filter today) and `event-header` (the host's and the guest's event header, a centred version against the left edge he chose); `privacy-hero` round three as a new concept ("the image trail was a takeaway win... maybe more fitting for its theme").
