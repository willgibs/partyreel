...
- Design: the Aurora's field (`SectionLight`) has no production call site; the home's two (the guest ledger and the closer, `second=home-arc`) land the moment Will answers the light board's `landing` step. The throw's call site (the QR plate) and the publish flourish (`rxp-bloom`, `rxp-pubglow` to a bloom in the house five, or sampled where it bleeds from media) wait on the bloom card.
