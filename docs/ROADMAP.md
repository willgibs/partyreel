...
- From `guest-chrome-wiring` (2026-09-20): the guest album's control row still holds only Download all; the shared `ViewMenu` (tile size and the Yours filter beside the mark's own tap) is a one-line mount there, the lane having handed off before `controls-home-wiring` landed.
...
- From `admin-wiring` (2026-09-20): the components gallery draws two "Surfaces" blocks (a duplicate React key on every load of `/design/library/components`; one section name fixes it); the help centre's palette (`marketing/help/help-palette.tsx`) predates `ui/command-palette.tsx` and keeps its own combobox and a second global ⌘K listener (fold it onto the primitive); the queue's Reports row costs a full `listReports("open")` for one timestamp (an `oldestOpenReportAt()` head-read), the same for the two inbox lists; the two report verdicts are still bare destructive buttons with no confirmation (admin-triage's lane; the sheet reaches them); a paid-subscriber delta needs history the webhook does not keep (a tiny `tier_events` table, his call) and the `admin_actions` operator log is proposed in the lane's Handoff, not built.
