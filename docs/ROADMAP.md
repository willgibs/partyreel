...
- From `guest-upload-wiring` and `first-event-wiring` (2026-09-21): the guest album's arrival hold lives in `src/lib/shared/arrival.ts` (`useArrivalMarks`) and the host hub keeps its own from the live-hub wiring; the next lane that touches the hub folds it onto the shared hook so both surfaces hold an arrival for one length.
- From `first-event` (2026-09-21, Will on `venue=sheet`, verbatim: "This is a great spark to a bigger idea. We should have a full gallery of printable QR designs ready to go, rather than only offering the code itself. Would make it very easy to use quickly for events, rather than always requiring the host to design the rest of the assets."): the print stock ships in one design (`first-event-wiring`); the gallery of designs is a board once the desk is closed (his rule: no new board now), the stock's designs as a catalogue to pick from.
- From `guest-upload` (2026-09-21): the terms line at the upload sheet says the product's limits, not the host's own `max_upload_bytes`, because the `get_event_by_qr_token` RPC does not return it (a migration adding the column, the types regeneration, `src/lib/db/queries/guest-events.ts`); then the line says the host's number, as the option he chose reads.
- From `first-event` (2026-09-21, `first=live`): the gallery doorbell trigger rings only when the approved-visible set changes, so a pending upload never wakes the host's hub; `first-event-wiring` bridges it with a host fingerprint route polled on the guest's cadence; a host channel rung on every arrival (a migration on `media_gallery_doorbell`) would retire that poll.
- From `app-pricing-wiring` (2026-09-21): eleven help articles describe buying on the pricing page where the sheet now opens in-app (`upgrade-downgrade-or-cancel.mdx` eight times; `password-protect-your-event.mdx`, `event-settings-explained.mdx`, `what-you-can-upload.mdx`, `what-the-free-plan-includes.mdx`, `what-happens-when-storage-fills-up.mdx`, `payments-receipts-and-invoices.mdx`, `pro-vs-event-pass.mdx`, `your-dashboard-explained.mdx`, `storage-plans-and-limits.mdx`; `custom-event-link.mdx` fixed in the lane): a `help-sync` lane rewrites them against the tree. No Library specimen yet for `PricingSheet`, `LockChip` and `WelcomeToPro` (a live demo would put a real Checkout door in the lab): a static specimen when the gallery can mock the door.
- From the components gallery fix (2026-09-20): `/design/library/components` logs a next/image warning on the hero specimen (`mkt-wedding-golden-01.jpg` with `fill` and `sizes="100vw"` not rendered at full width); the Library's own hero plate wants a real `sizes`, one line in the marketing gallery's demo.
- From the closing sitting's second batch (2026-09-20, Will's `phone=stack` note): a swipe row of cards (the board's `snap-x` row with the next card peeking) is banked for the marketing site's future gallery-type sections, never the plan cards.
- From `controls-home-wiring` (2026-09-20): the View menu's Sort ships DISABLED because the hub gallery's client holds one page, not the whole approved list; the day the list is whole (or the sort moves server-side) the two orders switch on; and `content/help/your-event-page-explained.mdx:33` describes the old control row (the next `help-sync`).
- From `guest-verify-r2` (2026-09-20, the lab): the project's CONFIGURED Auth rate limits were read from the dashboard on 2026-09-21 (Authentication, Rate Limits, in Will's Chrome under his permission): emails 100 an hour project-wide, OTP and magic-link verifications 150 per 5 minutes per IP (1,800 an hour), sign-ups and sign-ins 150 per 5 minutes per IP, token refreshes 150 per 5 minutes per IP, anonymous sign-ins 30 an hour per IP; the board draws Supabase's current documented defaults (verify 360 an hour per IP with bursts of 30; otp 360 an hour project-wide; emails 30 an hour on custom SMTP, raisable) labelled as such, and the read numbers swap in through `call:numbers` without a redraw, with the wiring of round two.
- The app round (Will, 2026-09-19: the host app and the guest pages unprotected, "closer to a Frankenstein's monster"), cut on freed seats in order, all four integrated and on the desk (`app-shape`, `guest-shape`, `privacy-concept` (round three, a new concept: "the image trail was a takeaway win... maybe more fitting for its theme"), `app-vocabulary`); his tile-size control lives in `app-vocabulary`, his centred event header in `guest-shape`; the seams are in `tracks/orchestrator.md`. One finding to fix regardless of the boards: `guest/file-dropzone.tsx` is a relic rendered only by the host's manual add (the `/u/[slug]` 404 landed with `profile-wiring`).
...
- From `toasts` (2026-09-20, the lab): `lab:demo` reports a step FROZEN at 0.00 percent when its options differ only inside grid-stacked, visibility-toggled `srcdoc` iframes: the captured screenshots come back byte-identical whichever option shows (reproduced in the interactive pane too), the twin of the glass backdrop-filter finding; verified by hand three ways on `toasts.material` and `toasts.action`; until the harness captures each frame on its own, such a step is judged by hand and the gate reads its demo exit with that in mind.
- From `home-states` (2026-09-20, the lab): if `app-shape`'s `busy=collapsed` is picked, `next-step-band.tsx` needs a real collapse built into the shipped component; the round's `CollapsedNextStepBand` (`sandbox/app-shape/home.tsx`) is a lab-only proof over the same `resolveNextSteps` output, never a fork of the rule and not a component to import as-is.
- From `help-sync` (2026-09-20): no help article mentions the demo (`content/help/` grepped clean after `demo-wiring` landed); nothing is false, so a future round picks the slug and the audience deliberately rather than a rewrite lane guessing one.
- From `guest-verify` (2026-09-20, the lab): the lab's pinned stage crushes its own head at 375 when a step has ONE config row (the recommended-option line and the knob row share a line and the label truncates to "the board s... O..."; reproduced on `seed-avatar.look`, so it is `src/components/lab/`, not a board; `lab:demo` passes it because it measures reach and clipping, not the head).
- From `buttons-pairs` (2026-09-20, the lab): a board-page measurement effect keyed on a module-constant probe array freezes when the board's own control swaps the drawn option without remounting (`BoardSection` hands one unkeyed child); fixed on `body-type` by threading a change token into the effect; any board that measures on the page keys its effect the same way, or the kit keys the child.
- From `pricing-fit` (2026-09-20, the lab): a fully ruled ask is unreachable by `?session=<board>.<ask>` even by a direct link (`_desk/queue.ts`'s `boardWork` walks only an ask with no ledger answer), which contradicts `lab-demo.mjs`'s own comment that a ruled step stays measurable; one condition in `boardWork`.
- From `guest-shape-r2` (2026-09-20, the lab): `ui/sheet.tsx` has no centred desk posture (a side panel only, `sm:right-0`); if a later consumer wants a door centred at a desk on the same primitive, a `desk` prop is the line (the round measured the shipped posture honestly and recommended `card` partly because of it).
- From `vocab-wiring` and `pricing-wiring` (2026-09-20): `/pricing` opens on paper but the cinema layout pins `themeColor: #040405`, so a phone's browser chrome is dark over a white first screen (the layout's note forbids a per-page `viewport` export: a group-level answer); `shared-band.tsx` is drawn only by the `pricing-page` board and goes when it retires; the tile-size cookie (`pr_tile_size`) is the mechanism where the ruling said localStorage (his to overrule); `navigation-menu.tsx`'s cross-slide onto `floatingCrossSlide`.
- From `overtaken-2` (2026-09-20, the lab): `first-event.first`, `.asks` and `.limit` are reached by the sixth batch (`guest-shape`'s `live=land` builds the arrival on the guest's side, the machinery `.first` asks the host's page to buy) and unbadged, because `_desk/queue.test.ts` pins first-event at four overtaken (`toHaveLength(4)` twice near its lines 368 to 370, and the `overtakenOpen` array at 406 becomes `["first", "hand", "landing"]`): three lines and three map entries in one stroke, the Orchestrator's; `navigation-menu.tsx`'s cross-slide still spells `data-[motion=...]` inline and swaps onto `floatingCrossSlide` (identical values) once a lane owns it.
- From `glass-wiring` and `door-wiring` (2026-09-20): the `PasskeysCard` line on `account/page.tsx` (one line under the Password card, after `avatar-wiring` lands); the magic-link half of `existing=tell` (a one-line banner on `/dashboard`, `home-wiring`'s file; `checkExistingAccount` is the shared rule); a Google-only host is never remembered on /login (the device memory is written only where the address is known; the passkey is the real answer, wired and dark); `host-app.md`'s "three chips" sentence on the host tile is now a desk-only row (the Orchestrator or `help-sync`); the `glass-mark-lit` halo if it reads as a shadow (the alternative a darker tint everywhere dark).
- From `guest-wiring` (2026-09-20): the arrival's ruled growth ("a new photograph grows into ONE column and only that column re-flows") needs `MasonryColumns`' explicit column assignment, landed on top of `glass-wiring` (the glow ships meanwhile); Download all (`ExportDialog`) and Save (`SaveEventButton`) onto the responsive Sheet, the other two of `dialogs=stands`' four, both shared with host surfaces; the Sheet's phone half on a real iPhone (a focused textarea; the home indicator: the constant carries no `env(safe-area-inset-bottom)`), and if either bites the phone half becomes vaul-backed once in `ui/sheet.tsx`; a component pin for `LiveGallery`'s own-photograph wiring (the routes, the pure arrival ids and the lightbox gate are pinned; the component joining them is not).
- From the sixth batch (2026-09-20): passkeys ship behind `NEXT_PUBLIC_PASSKEYS` on Supabase's experimental API and stay off until Will enables passkeys in the Supabase dashboard (Auth) with the RP id on the apex; the responsive Sheet's phone half has never held a focused input on a real iPhone (Save and Report join it under `guest-wiring`; vaul's `repositionInputs` is the fallback engine for that half); the operator audit table (`admin_actions`) is the `arm` grammar's and stays a proposal in `admin-wiring`'s Handoff; the tile-size pick persists in a cookie for an honest first paint though the option's own word was localStorage (his to overrule); the two legal lines that say an anonymous uploader asks the host to remove a photograph go stale the day `guest-wiring` lands (`legal-privacy.tsx:515-520`, `legal-terms.tsx:123`: `help-sync`); the lab's `pricing-page` phone step demo is broken (his note) and is repaired in `pricing-fit`.
- The stacking round (Will, 2026-09-19, while deployments are capped: "explorations are, at worst, net neutral and deleted"): six boards on the seats, `demo-event`, `app-door`, `contact-page`, `press-page`, `app-pricing`, `pricing-page`, and two queued from the Orchestrator's read of the weak points, `guest-upload` (the core act, phone first) and `first-event` (from "Create" to a code on the table); each was cut from a read-only map whose seams are in `tracks/orchestrator.md` ("The app round's map"). Findings to fix regardless of the boards: the press kit's mark and icon plates are the retired Aperture glyph (ASSETS row 19); "Availability: Live now" is ungated on a pre-launch branch; the demo's bulk export APIs hide `isDemo` in the UI only; the careers form re-implements `/contact`'s contract in parallel.
- The overnight round (Will, 2026-09-19, asleep: "occupy 8 more slots, paced as usual", then "12 more agent slots throughout the night"): twelve boards at the Orchestrator's discretion, six seats at a time, `media-viewer`, `reel-studio`, `host-curation`, `admin-triage`, `emails`, `help-center`, then `profile-page`, `how-it-works`, `export-flow`, `site-chrome`, `event-type-pages`, `error-pages`; each cut from a read-only map whose seams are in `tracks/orchestrator.md` ("The overnight round's maps"). Findings to fix regardless of the boards: the notification card's four switches for mails that do not exist; the renewal nudge's "Renew Event Pass" button linking `/dashboard`, not Checkout; `report_status` "reviewed" never written while marketing promises every report is reviewed; the Review peek's comment promising an Escape no handler honours; no guest surface links to `/help`.
- A partners page and program for planners before launch (Will, 2026-09-19: "I plan to create a partners page/program in the near-term before launch, that we can point to for planners"); until then planners read the site as hosts, and no page greets them apart.
- From `glass-material` (2026-09-20, the lab): a `Page.captureScreenshot` with a `clip` makes Chrome recomposite at a new surface size and the backdrop-filter layer does not survive it (every clipped pane came back pure black, 20.49:1 against white on all nine states), so a harness that measures glass captures the viewport and crops in Node, and `lab:demo`'s UNPAINTED report is the same disease one layer up; a HEADED Chrome capture drops composited layers where headless does not (twice, deterministically, the paused reel and two tiles of the host's row gone from the shot while the DOM measured them present), so the record captures are headless with a live headed reading beside them; for the glass wiring, the rose `--like` active mark fails over a bright photograph on EVERY material (1.44:1 Frost, 1.01 Crystal, 1.01 White, measured), so the mark itself must carry its own contrast, which reaches `media-viewer`, `host-curation` and `app-vocabulary` under his tiles rule.
- From `host-curation` (2026-09-19, the lab): the hidden-media dim has never rendered: `masonry.tsx:198` builds `active:scale-[0.98]${dimItem ? "opacity-30" : ""}` with no separator, so Tailwind never emits the token and a hidden photograph sits in the host album at full brightness under a comment (and `host-app.md`) describing a 30 percent dim; one consumer, no test. Later: `ApproveAllPendingButton` has no caller; the lightbox's pending Approve branch can never render; the review peek promises Escape in a comment and never listens; the review queue is `grid-cols-3` at 375 while the album is `columns-2`; the settings' Deleted card is invisible until something is in it; no test covers `useReviewTriage`, `ReviewActions` or the bulk mutations.
- From `error-pages` (2026-09-19, the lab) after `errors-wiring` (2026-09-19): the group 404s render in a fixed `min-h-[60vh]` box rather than filling the screen (its own small decision); a `render:root` Sentry area so the root `error.tsx` and `global-error.tsx` stop sharing `render:global` (one line in `src/lib/observability/sentry.ts`, one in `route-error.tsx`); a second boundary probe for `global-error`, since `/design/lab/tools/boom` now lands on the root boundary and the last-resort screen has no probe left (a `?boundary=global` that crashes the ROOT layout); `/design/library/patterns`'s `RouteErrorMock` still draws the old digest chip and no help line; one copy-with-a-receipt primitive for `marketing/press/copy-button.tsx` and `shared/error-digest.tsx`; a runbook page for the operator turns "Check the runbook." into a link (`admin/not-found.tsx` and `HELP_BY_AREA`'s admin row); in the lab, stacking several chromes with `vh` minimums inside one `Frame` compounds against the frame's height (the board's `standalone` prop was the pattern). Found on the alias (2026-09-19): `/dashboard/<not-a-uuid>` crashes into the app's boundary with a Postgres `22P02` (two server events and one `render:app` per visit, old Sentry issues) where `notFound()` is the truth, so `getEvent` in `src/lib/db/queries/events.ts` should refuse a malformed id before the query; and a group 404 reached through a dynamic marketing route (`/help/nope`) carries the bare `Partyreel` title while the root and paper 404s say `Page not found`.
- From `event-identity` (2026-09-19, the lab; ruled `door`, so ASSETS row 26 is withdrawn) and `events-wiring` (2026-09-19): a per-type highlight render stays an idea (the door's reel column shows one shared cut on four pages); `SectionShell`'s subhead carries no size class (16 px inherited) while `PageHero`'s rides the `subhead` step, and `body-type` owns whether the section slot joins the ladder; `AlbumStream` takes one fixed photograph set, so the falling hero cannot be themed per event type without a frames prop (the cost `hero-theme=arrival` was drawn carrying).
- From `event-type-pages` (2026-09-19, the lab): the reunion post carries the `parties` audience tag while its subject is a trip and `blog-tags.ts` has no `trips` tag; three posts carry an audience tag with no link into a type page; the mega panel and the footer hand-write a third description per type beside `EVENT_TYPES.teaser` (consolidate in a lane that owns `marketing-nav.ts`; `events-wiring` gives `events.ts` the per-type media ids); `src/lib/constants/events.ts` sits outside the content policy's claim scan (the stale comment, the hub's OG image and the doc line landed with `events-wiring`, 2026-09-19).
- From `how-it-works` (2026-09-19, the lab) after `loop-wiring` (2026-09-19): `content/help/how-partyreel-works.mdx` still tells FIVE steps where the page and the home's stepper walk six out of `src/lib/constants/how-it-works.ts` (the mega panel's card stopped counting rather than claim a number wrong on one of them); the help lane brings the article's `<Steps>` onto the same six. The home's how-it-works passage renders as `id="how-it-works"` but `section-ids.ts` and the file still call the slot `film-strip`; rename the slot and the file in a lane that owns `sections/home/section-ids.ts`, `index.ts` and `home-sections.test.ts`.
- From `site-chrome` (2026-09-19, the lab) and `chrome-wiring` (2026-09-19): the footer's Product column hard-codes FAQ to `/#faq`, an anchor only the home has, so the row leaves `/pricing` (which has its own; left alone because `marketing-nav.ts` was read-only in the lane); Tab into the hidden bar costs the reader about 482 px of scroll position (Chrome's scroll-into-view giving up against a sticky element; Shift+Tab scrolls up first, so rare; a keydown-on-Tab return would pre-empt it); the phone sheet's foot actions (Log in, Start free, Dashboard) carry no `trackAttrs` while the header's do; the nav labels sit at `text-muted-foreground` under the transparent bar over a bright cinema hero (the balloons frame at 1440) and are hard to read before the glass arrives; `logo.tsx`'s `markOnly` branch is a placeholder tile with no caller (it goes the day the v1 icon lands, ASSETS row 19).
- From `profile-page` (2026-09-19, the lab) and `profile-wiring` (2026-09-19): `/admin/reports` has no way to reach a reported PERSON's account (suspend, clear a bio, remove a handle), so the operator acts out of band and only closes the report; the profile's event cards presign the cover's ORIGINAL (`/original.jpg`, 1920 wide, multi-megabyte, `loading="lazy"`), so a card paints black for seconds on a laptop where a preview derivative would land at once (the same read the dashboard's cards make; measured on the alias 2026-09-19); an attended card whose guest added only video draws `EventCard`'s lock fallback (`href: null` is what an unopenable card looks like) and deserves its own empty face; `src/lib/validation/report.test.ts` has no person-arm cases (the schema's refusals were exercised live: both subjects, neither, a cross-subject `media_id`, all 400); three hand-rolled toggles do one job (`FollowButton`, the menu's block, the Connections card's buttons: one control, one contract); the "Someone followed you" switch promises mail no code path sends; the profile overflow opens over the person's own name at 375. Major overhauls: the follow graph has no consumer worth the graph ("Following" is one dashboard chip and nothing else reads a follow).
- From `export-flow` (2026-09-19, the lab): the export mint has no timeout and no cancel (a hung request leaves the toast spinning and Download disabled until a reload); the Worker skips an R2 object it cannot find in silence (an album emptied between mint and stream downloads as a valid, empty zip); the zip mock says the caps are "deliberately unmentioned" while the album copy states "Up to 2,000 items" and the dialog says no number (all three must agree); the dialog prints raw integers ("2440 items"); `/admin/exports` has no heartbeat; nothing says whether the form-POST attachment saves on iOS Safari (one real iPhone answers it); the bulk bar's "Download selected" mints with hidden items in and no confirmation (`app-vocabulary`'s `bulk-toolbar`).
- From `admin-triage` (2026-09-19, the lab): `report_status`'s `reviewed` is written by no code path while the Terms, the privacy policy, `features.ts` and `events.ts` promise "every report is reviewed" (a verdict writes it or it leaves the enum); `reports.resolution_note` has never been read or written since the founding migration (the `verdict` decision fills it); `ReportReviewList` and `ModerationGrid` import live server actions at module scope (take the action as a prop, as `TriageStatusControl` does); `/admin/reports` shows no count, no bulk act and no phone layout. The lab: a responsive Tailwind variant (`sm:w-[200px]`, `hidden lg:inline`) never reaches a board's frame while arbitrary values do; a board needing a breakpoint writes a media query in its own sheet, and the root cause is worth finding before a wiring round promotes anything from one.
- From `media-viewer` (2026-09-19, the lab): the viewer's own image and video have no loading state (a tile has a skeleton; the opened photograph pops in when the full-size presign lands, the slowest picture in the product on venue Wi-Fi): whichever `opening` wins, the wiring owes it an arrival; `MediaTile` carries `loading="lazy"` and Blink resolves it against the TOP window even inside a same-origin iframe, so a strip below the fold never loads (a gallery tile used as chrome is a different job); six surfaces disagree about what a viewer holds (the admin passes Save only, the bin everything but Save, Uploads has no Share); `/api/reports` accepts a `media_id` the dialog never sends; the Review peek is a third full-bleed viewer.
- From `emails` (2026-09-19, the lab): all ten shipped mails are HTML-only with no `text/plain` twin (additive: a `text` string beside each `html`); `sendOnce`'s claim-then-send dedupe has no direct test (a mocked-Resend Vitest or a rolled-back Supabase-MCP check closes it); `docs/PRICING.md`'s "Follow-up (code)" line says the resend button has no cooldown while `email-sign-in.tsx` ships `RESEND_COOLDOWN_S` (delete the line). The renewal button's destination and the four dormant switches are answered by the board's `foot` and `moments` decisions; the wiring closes them.
- From `reel-studio` (2026-09-19, the lab): `StyleRail` has zero callers since the Studio took `StyleWall` (revived by the `styles` ruling or deleted with the wiring); the 360 / 640 player cap is a literal in four files (`player.tsx`, `reel-studio.tsx`, `guest-reel-overlay.tsx`, `guest-reel-card.tsx`) and wants one source before the `room` ruling moves it; `PosterCard` spreads to its column while the player caps at 360, so the host's Reel section draws a 1280 px card around a 360 px reel at 1440; `host-app.md` still names the render poll pruned 2026-07-08; the cover strip is the one room tile not on `UNIFORM_TILE_ASPECT`. Later: the moments picker's pool uses a viewport breakpoint, so a sheet in a 1440 room shows five of thirty tiles.
- From `help-center` (2026-09-19, the lab): no help article emits `FAQPage` JSON-LD though every one is a clean question-and-answer pair (per-article at zero authoring cost); the audience default (host unless `guest-experience` or `troubleshooting`) guesses wrong more often than right in two categories, worth a per-article pass; the `article.screen` option wants a maintained illustration per step, so a wiring round adopting it asks for real per-step screenshots of the shipped surfaces and the beacon wants one small table with a per-article count in `/admin`.
- From `contact-page` (2026-09-19, the lab): `contact-sheet.tsx` and `press-sheet.tsx` are photography proof sheets, not contact surfaces (a cheap rename when either is next touched); the 2026-08-28 composite ruling on the desk (`contact-identity`) predates rulings.md and has no entry there (backfill it verbatim next time a ruling is appended); the honeypot field is named `website`, a real field elsewhere, and `actions.ts` has no end-to-end test.
- From `guest-upload` (2026-09-19, the lab): the hold-for-approval toast and the floating Add pill both sit at the foot of a phone and the toast covers the pill; whichever `held` answer wins, the wiring owes the two surfaces one stacking rule.
- From `first-event` (2026-09-19, the lab): the product's QR plates are typed pixel numbers while the marketing river's plate computes a module floor (`qrPlateFloorPx`): the wizard's and the QR designer's 96 px swatches render at 2.3 px a module (2.05 on the two Q presets), under the 3 px floor, so a host cannot test-scan the style they are choosing, and the style step's preview link is `/e/` plus 32 zeroes, a 404; give the plates the river's `max()` treatment and encode the real token or say the swatches are samples. One CTA label for creating an event (three today) once `voice` rules. If `venue=sheet` wins it is the share studio's own track (paper sizes, cut marks, a print stylesheet, a PDF path).
- From `app-door` (2026-09-19, the lab), app polish under any answer on the board: the Save and Likes account prompts create accounts with no Terms line (0 of 2 guest surfaces carry it, measured); `login-form.tsx` redraws the Google "G" by hand beside the shared `auth/google-icon.tsx`; the signup's "Pick a password" step has no strength meter while `/account`'s change form does.
- From `demo-event` (2026-09-19, the lab): `DemoTicket`'s `row` variant is dead code and its comment contradicts `cinema-hero.tsx`'s (each says the other renders it); whatever `doors` answers, one comment is wrong today. Launch checkpoint: the demo's photographs are placeholders and the `event` decision sets how many albums the Higgsfield month produces (one, or three). Security: the demo's per-tile Save and Share and the bulk export routes enforce no `isDemo` server-side (the UI hides them; a request does not have to); decide whether the demo's capability token carries a read-only claim.
- From `press-page` (2026-09-19, the lab): the fact sheet's "Live now" reads true of the product's availability whichever option wins (the lane's recommendation: leave it; a one-line edit to `press.ts` if Will disagrees).
...
The lab and the kit:
- The Library: `HowItWorksStepper` (`sections/shared/how-it-works-stepper.tsx`) has no specimen (a full-width section, carried as `unspecimened`); a `gallery-demos` entry would let the Library show it (`loop-wiring`, 2026-09-19).
- Banked (Will, 2026-09-19, `the-cards=frame`): the conference and trip cards at 375 with the artifact slightly tucked above the copy floor, "a nice design, maybe useful for something else where the visual could tuck"; the drawing stays in git under `sandbox/event-identity/cards.tsx` after the board retires.
...
- ★ `glass` round two is `glass-material` (2026-09-20: the ONE material, Frost, Crystal or White, measured on every glass surface at once, the wiring waiting for it, his choice); marketing glass (the header's `GlassLayer`, the overlays and the set-pieces over photographs) is a later round once the app's material is ruled.
...
- Product gaps the help catalog documents honestly: the video-uploads wrapper string omits the Event Pass; `?upgraded=1` is never read; "Public" in settings vs "Open" in the header chip; the restore toast never reads `mediaStillRemoved`; the ops-only missing-ETag error can reach a guest; the privacy FAQ overstates the event-level Report; `tiers.ts`'s comment cites a retired video limit; the open-event unfurl promises "no account" when the event requires one; `lifecycle-recovery.md` vs `listRecentlyDeletedMedia` on a guest's self-deleted upload.
...
- From `ladder-wiring` (2026-09-20): `src/components/admin/` is owned by no manifest; the `admin` board's wiring should claim it (three of its sizes sit in the allow-list under `board`).
- From `ladder-wiring` (2026-09-20): `rules.generated.json` was 36 contract line numbers stale on `launch-prep`, so every lane's `pnpm design:rules` touched a file it does not own; the Orchestrator regenerates it at every record (the merge script does) and checks it in.
...
- From batch five: `SITE_DESCRIPTION` (thesis + subhead) reaches about 200 characters with the new hero sentence; `voice-wiring` gives the meta description its own line under 160.
- The lab imports fifteen production modules the wiring lanes reshape (`filter-chips`, `trash-section`, `storage-meter`, `feed-section`, `empty-section-teaser`, `event-card`, `app-shell`, `feed-section-header`, `event-filter-pills`, `review-section`, `use-review-triage`, `recently-deleted-grid`, `event-settings/*`, `event-slug-control`, `my-uploads-gallery`); tonight's rule is never delete or break them; the fix is fixtures under the lab (composition-demos, gallery-demos, the boards).
- After `home-wiring` and `hub-wiring` land: a `help-sync` lane rewrites the help articles that describe the dashboard, the event page, sharing and settings (each lane lists them in its Handoff; help how-tos track shipped reality).
...
- A demo event set and ready on every environment (Will, 2026-09-19: "we should always have a demo event set and ready"): no guard exists on `NEXT_PUBLIC_DEMO_QR_TOKEN`, which is inlined at build, so a deploy with it missing ships a footer without the code; the footer's Start free stops depending on it (`chrome-wiring`).
