...
- Code hygiene: comments that still name save: `likes-provider.tsx:26` (SaveEventButton), `queries/guest-events.ts:19` (`save_event` among the anon-client RPCs).
...
- Verify a change-plan session `[eng]`, TEST now and live at the cutover: a Pro host's Change plan opens Stripe's confirm page for one price at quantity 1 and returns to the app; read the session back per billing-caps.md's runbook, and look once at what Back from the confirm page shows (never that configuration's home with its switcher). TEST needs a real subscription first: willg97 is Pro by hand, with no customer.
