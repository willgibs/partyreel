...
- The reel: once `node scripts/sweep-reel-files.mjs --apply` has run on both buckets (again with `R2_BUCKET=partyreel-backup`; one stored reel in each today) and dry runs report zero, remove `reelOutputKey` (`src/lib/r2/keys.ts`), the purge cron's append and `account-deletion.ts`'s append (from `reel-teardown`); the comments still naming the dropped tables go with them (`scripts/seed-demo-event.mjs`, `r2/keys.ts`, and the reel engine's `reel-types.ts`, `style-registry.ts` and `themes.ts`).
...
- **The reel:** the stretch landed on 2026-09-25 (guest, host, clip, teardown, sweep; the alias's build 9); what is left is the one-shot R2 sweep of the stored reel files in both buckets (its Now line), then `reel-marketing` after his `reel-story` r2. Ideas at zero storage, since a reel is a recipe:
