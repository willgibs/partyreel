...
- From `error-pages` (2026-09-19, the lab): the root layout has no `error.tsx`, so a crash inside a group's OWN layout skips every branded boundary and lands bare on `GlobalError` (add `src/app/error.tsx` once a shape is picked); the group 404s render in a fixed `min-h-[60vh]` box rather than filling the screen (its own small decision, untouched by `surround`); in the lab, stacking several chromes with `vh` minimums inside one `Frame` compounds against the frame's height (the board's `standalone` prop is the pattern).
...
- From `how-it-works` (2026-09-19, the lab): the loop is told four ways with three counts (six steps on the page, five in `content/help/how-partyreel-works.mdx`, "four steps" in the mega panel's card, three on the home strip) until the winning count is wired everywhere the loop is told; "How Partyreel works" links the article from the spine and the mega panel and this page from the help hub; `src/lib/constants/how-it-works.ts` calls itself the single source and only the welcome flow reads it while `spine.tsx` hand-rolls six steps; `docs/systems/marketing-content.md`'s page catalogue has no entry for `/how-it-works` (add one regardless).
- From `site-chrome` (2026-09-19, the lab): the footer's Product column hard-codes FAQ to `/#faq`, an anchor only the home has, so the row leaves `/pricing` (which has its own); the Resources card says "four steps" against the article's five (the count is `how-it-works`'s, the copy the chrome's); the nav labels sit at `text-muted-foreground` under the transparent bar over a bright cinema hero (the balloons frame at 1440) and are hard to read before the glass arrives; `logo.tsx`'s `markOnly` branch is a placeholder tile with no caller (it goes the day the v1 icon lands, ASSETS row 19).
