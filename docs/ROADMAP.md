...
- The app round (Will, 2026-09-19: the host app and the guest pages unprotected, "closer to a Frankenstein's monster"), cut on freed seats in order and all four cut (`app-shape`, `guest-shape` and `privacy-concept` (round three, a new concept: "the image trail was a takeaway win... maybe more fitting for its theme") on the desk, `app-vocabulary` building); his tile-size control lives in `app-vocabulary`, his centred event header in `guest-shape`; the seams are in `tracks/orchestrator.md`. Two findings to fix regardless of the boards: `/u/[slug]`'s 404 falls through to the marketing chrome (no `(guest)` not-found boundary), and `guest/file-dropzone.tsx` is a relic rendered only by the host's manual add.
