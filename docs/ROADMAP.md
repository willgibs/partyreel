...
- Type: one FAQ, one look. The home and /pricing accordion's questions are on the card step (Urbanist 16/600) since `ladders-wiring`; the shared `FaqAccordion` (/events, the feature pages) sets its questions as Inter 14/500 in a `<summary>` that is not a heading, so the two FAQs read differently.
...
- The lab: two leftover forced-h-11 CTAs (`sandbox/album-hero/hero.tsx`, `sandbox/glow-moments-variants.tsx`) take `size="cta"` if their boards outlive the wiring, and `lab/tools/motion/motion-playground.tsx` still names `/design/lab/rounding` as a specimen; re-point it at `/design/library/foundations#radius`.
