...
| **Guest flow** | The `/e/[token]` event page: 3-state visibility, capability tokens, password/unlock, the door (a name, an optional email, the host's gates), the live gallery, demo mode, the guest reel | [guest-flow.md](systems/guest-flow.md) |
| **Host app** | The host's surfaces: the dashboard, the create flow, the QR designer and print, custom slug, first-time welcome, the event page and settings, host-side moderation/curation, the host-upload entry | [host-app.md](systems/host-app.md) |
...
| **Notifications · analytics · growth** | The derive-on-read host bell, `link_stats` (aggregate, no PII), the marketing web analytics, saved events, guest email capture | [notifications-analytics-growth.md](systems/notifications-analytics-growth.md) |
