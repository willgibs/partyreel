...
The deep docs live in [`systems/`](systems) (start at [`systems/README.md`](systems/README.md)); each is
one Read with what-it-does · where-it-lives · invariants · gotchas · ADR links.
...
| **Profiles & social** | Public creator profiles (`/u/[slug]`), the follow/block graph, the host-controlled guest list, notification-pref storage (migration pending integration) | [profiles-social.md](systems/profiles-social.md) | 0019 |
...
`reel_eligible`/`preview_key`) but **no processing ships**. Transcode/stitch must run in an **external
worker, NOT Vercel** (ADR-0003). Tabled pending a product + architecture decision (the worker platform) —
see [ROADMAP.md](ROADMAP.md) and the host-app stub.
