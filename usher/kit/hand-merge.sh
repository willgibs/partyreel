...
#   the board rows (touchpoints.ts): UNION of both sides (each lane rewrote its own row), then any retired id the
#     union re-added to DESK_ORDER is dropped (the typecheck caught this once);
...
TP="src/app/(dev)/design/touchpoints.ts"
for f in "${CONFL[@]}"; do
  [ -n "$f" ] || continue
  case "$f" in
...
PY
      ;;
    "$TP") git show ":1:$f" > "$S/hm-base"; git show ":2:$f" > "$S/hm-ours"; git show ":3:$f" > "$S/hm-theirs"
...
PY
grep -l "<<<<<<<" "$REG" "$BOARDS" "$TP" 2>/dev/null && { echo "STOP: markers remain"; exit 3; }
node "src/app/(dev)/design/gallery/collect-specimens.mjs" >/dev/null 2>&1 || { echo "STOP: the specimen collector failed"; exit 3; }
[ -f "docs/tracks/$TRACK.md" ] && git rm -qf "docs/tracks/$TRACK.md"
git add "$REG" "$BOARDS" "$TP" "src/app/(dev)/design/gallery/specimens.generated.json" 2>/dev/null
pnpm -s typecheck > "$S/$TRACK-typecheck.log" 2>&1; TC=$?
pnpm -s vitest run "src/app/(dev)/design/touchpoints.test.ts" "src/app/(dev)/design/sandbox/registry.test.ts" "src/app/(dev)/design/(shell)/lab/_desk/queue.test.ts" src/lib/track-manifests.test.ts > "$S/$TRACK-registry-tests.log" 2>&1; RT=$?
