...
RC=1
for a in $(seq 1 $N); do
  perl -e 'alarm 420; exec @ARGV' pnpm -s lab:demo --board "$BOARD" --base http://localhost:3130 2>&1 | grep -v -- "$FILTER" | tee "$S/demo-rerun-$BOARD-$a.log" | tail -6; RC=${pipestatus[1]}; echo "EXIT[lab:demo $BOARD attempt $a]=$RC"
  [ "$RC" = 0 ] && break
done
