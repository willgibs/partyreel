...
Surveyed 2026-09-20 against the desk as it stood that night (28 boards; 27 on the afternoon of the 20th after admin, glass and voice retired and guest-verify and toasts joined). A candidate is cut only if it re-asks no open ask. STATE (2026-09-20, 14:00 EDT): Will's rule closes the desk before any new board; the eight briefs written from this table (`venue-screen`, `pwa-install`, `legal-reading`, `about-page`, `blog-reading`, `og-cards`, `feature-anatomy`, `llms-surface`, in the plan file under "The night's explorations, second") stay shelved until the desk is closed; `host-insights` is clean too. Excluded for overlap with open asks: `album-find` (guest-shape r2's theirs), `dark-surfaces` (emails.dark), `a11y-paths` (host-curation.keys, media-viewer), `album-afterlife` (export-flow.object, emails.moments), `notifications` (emails.moments), `likes-system` (media-viewer.holds), `billing-recovery` (app-pricing.back), `reel-marketing` (reel-studio.guests), `social-graph` (profile-page r2), `careers-apply` (contact-page). Every overlap check is rerun against the CLOSED desk before a cut: a closed desk changes what "today" means in every option.
